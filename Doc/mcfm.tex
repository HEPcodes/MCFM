...
\hline 
141 & $ H(\to b(p_{3})+\bar{b}(p_{4})) + b(p_{5}) (+g(p_{6}))$   & NLO \\
142 & $ H(\to b(p_{3})+\bar{b}(p_{4})) + \bar{b}(p_{5}) (+b(p_{6}))$   & (REAL) \\
143 & $ H(\to b(p_{3})+\bar{b}(p_{4})) + b(p_{5}) + \bar{b}(p_{6}) [\mbox{both observed}]$   & (REAL) \\
\hline 
151 & $ t(\to \nu(p_{3})+e^+(p_{4})+b(p_{5}))+\bar{t}(\to \bar{b}(p_{6})+e^-(p_{7})+\bar{\nu}(p_{8}))$   & NLO \\
152 & $ t(\to \nu(p_{3})+e^+(p_{4})+b(p_{5}))+\bar{t}(\to \bar{b}(p_{6})+q(p_{7})+\bar{q}(p_{8}))$   & NLO \\
153 & $ t(\to \nu(p_{3})+e^+(p_{4})+b(p_{5}))+\bar{t}(\to \bar{b}(p_{6})+e^-(p_{7})+\bar{\nu}(p_{8})) (uncorr)$   & NLO \\
\hline 
\end{tabular}
\end{center}
...
221 & $ \tau^-(\to e^-(p_{3})+\bar{\nu}_e(p_{4})+\nu_\tau(p_{5}))+\tau^+(\to \bar{\nu}_\tau(p_{6})+\nu_e(p_{7})+e^+(p_{8}))$   & LO \\
\hline 
231 & $ t(p_{3})+\bar{b}(p_{4})+q(p_{5}) [\mbox{t-channel}]$   & NLO \\
232 & $ t(p_{3})+\bar{b}(p_{4})+q(p_{5})+q(p_{6}) [\mbox{t-channel}]$   & LO \\
236 & $ \bar{t}(p_{3})+b(p_{4})+q(p_{5}) [\mbox{t-channel}]$   & NLO \\
237 & $ \bar{t}(p_{3})+b(p_{4})+q(p_{5})+q(p_{6}) [\mbox{t-channel}]$   & LO \\
\hline 
...
\hline 
280 & $ f(p_{1})+f(p_{2})\to  \gamma(p_{3})+f(p_{4})$   & NLO \\
...
\hline 
420 & $ W^+(\to \nu(p_{3})+e^+(p_{4}))+b(p_{5}) [\mbox{1 or 2 jets only}]$   & NLO \\
421 & $ W^+(\to \nu(p_{3})+e^+(p_{4}))+(b+\bar{b})(p_{5}) [\mbox{1 or 2 jets only}]$   & NLO \\
422 & $ W^+(\to \nu(p_{3})+e^+(p_{4}))+b(p_{5})+jet(p_{6}) [\mbox{2 or 3 jets only}]$   & NLO \\
425 & $ W^-(\to e^-(p_{3})+\bar{\nu}(p_{4}))+b(p_{5}) [\mbox{1 or 2 jets only}]$   & NLO \\
426 & $ W^-(\to e^-(p_{3})+\bar{\nu}(p_{4}))+(b+\bar{b})(p_{5}) [\mbox{1 or 2 jets only}]$   & NLO \\
427 & $ W^-(\to e^-(p_{3})+\bar{\nu}(p_{4}))+b(p_{5})+jet(p_{6}) [\mbox{2 or 3 jets only}]$   & NLO \\
\hline 
...
\ref{subsec:wt} below.
\item {\tt frag}.
Processes  285, 290, 295 300 only, see sections~\ref{subsec:gamgam}, \ref{subsec:wgamma} and
...
that determines the method used to calculate the scale:
  ...
  the $p_T$'s of all the particles in the event (with partons clustered into
  jets)
  \item {\tt 5d0} : $\mu^2 = \left( p_T^{5} \cdot p_T^{6} \cdot p_T^{7} \right)^{2/3}$
  -- the geometric mean of the $p_T$ of particles $5$, $6$ and $7$, appropriate for
  some processes, e.g. $W+3$~jets
  \item {\tt 6d0} : $\mu^2 = \left( p_T^{7} \right)^2$
  -- the transverse momentum of particle $7$, appropriate for
  some processes, e.g. $W+3$~jets
  \item {\tt 7d0} : $\mu^2 = \hat H_T^2$, where $H_T$ is the scalar sum of
  the $p_T$'s of all the particles (partons) in the event (i.e. no clustering into jets)
  \end{itemize}
  There is also a built-in facility for easily varying some of these scales by a constant
  factor about the central choice. These are illustrated in Table~\ref{dynamicscales}.
...
\hline
Central scale & $\mu_0^2 = M_V^2+p_T(V)^2$ & $\mu_0^2 = \hat H_T^2$ &
 $\mu_0^2 = \langle p_T^{jet} \rangle^2$ \\
\hline 
 $0.25 \times \mu_0$ & {\tt 11}  & {\tt 21}  & {\tt 31} \\ 
\hline 
 $0.5  \times \mu_0$ & {\tt 12}  & {\tt 22}  & {\tt 32} \\ 
\hline 
 $0.75 \times \mu_0$ & {\tt 13}  & {\tt 23}  & {\tt 33} \\ 
\hline
 $\mu_0$             & {\tt 14} ({\tt 1}) & {\tt 24} ({\tt 7})  & {\tt 34} ({\tt 2}) \\ 
\hline 
 $   2 \times \mu_0$ & {\tt 15}  & {\tt 25}  & {\tt 35} \\ 
\hline 
 $   4 \times \mu_0$ & {\tt 16}  & {\tt 26}  & {\tt 36} \\ 
\hline 
 $   8 \times \mu_0$ & {\tt 17}  & {\tt 27}  & {\tt 37} \\ 
\hline
\end{tabular}
\end{center}
\caption{Choices of the input parameter {\tt dynamicscale} that allow rescaling by a
constant factor about a central choice.}
\label{dynamicscales}
\end{table}
...
\item {\tt mtrans34cut}. Specifies the minimum transverse mass
...
fragmentation process is included. If {\tt frag} is set to {\tt .false.} the code implements
a Frixione-style photon cut~\cite{Frixione:1998jh} with parameters $\varepsilon_\gamma=1, n=1$ and $\delta_0$ set by
{\tt cone\_ang} specified below. Setting {\tt frag} to  {\tt .true.} will include fragmentation contributions at NLO. 


\item {\tt fragset}. A character*8 variable that will be used to choose the particular photon fragmentation set. Currently implemented fragmentation functions can be called with 
`BFGSet\_I' or `BFGSetII'~\cite{Bourhis:1997yu}.  
...
\item {\tt cone\_ang}. A double precision variable that fixes the cone size ($\delta_0$) for photon isolation. This cone is used in both forms of isolation. 
\item {\tt epsilon\_h}. This cut controls the amount of radiation allowed in cone when  {\tt frag} is set to {\tt .true.}. If  {\tt epsilon\_h} $ < 1$ then the isolation corresponds to isolating the photon using
...
user can employ processes 420--427 instead (see section~\ref{subsec:wbbfilter}).
These prcoesses use the same matrix
...
These processes represent the production of a $Z$ boson or a photon 
which subsequently
decays into $t \bar{t}$. The leptonic decays of the top quarks are included.
{\tt m34min > 0} is obligatory. The calculation may be only be performed at LO.
...
\subsubsection{$WZ$ production, processes 71--78}
...
Note that, in processes
{\tt 72}and {\tt 77}, a sum is performed over all three species of neutrinos.

Note that, in processes
{\tt 74,75,79,80}, the NLO corrections do not include radiation from the
quarks that are produced by the $Z$ decay. These processes will be observed
...
When {\tt removebr} is true in processes {\tt 117} the Higgs boson does not decay.

\subsection{$W^+W^++$jets production, processes 121,122}
These processes represent the production of two $W^+$ 
bosons in association with two (process {\tt 121}) or three (process {\tt 122})
...
\subsection{$t\bar{t}$ production with decay, processes 151--153}

These processes describe $t \bar{t}$ production 
including two leptonic decays $t \to b l \nu$ (process {\tt 151}) and 
one leptonic and one hadronic decay ({\tt 152}).
The computation may be performed at NLO, using the virtual amplitudes
presented in ref.~\cite{Korner:2002hy}.
...
In the case of process 153, there are no spin correlations in the decay of the 
top quarks. The calculation is performed by multiplying the spin summed 
top production cross section, by the decay matrix element for the decay of the 
$t$ and the $\bar{t}$.
...
\subsection{$H+$~jet production, processes 201--209}
\label{subsec:hjet}
...
When {\tt removebr} is true in processes {\tt 201}, {\tt 203}, {\tt 206}, {\tt 208}
and {\tt 209}, the Higgs boson does not decay.
...
\subsection{$t$-channel single top with an explicit $b$-quark, processes 231--237}
\label{subsec:stopb}
...
processes in a scheme with four flavours of quark in the proton, so that a $b$-quark is forbidden.
...
\subsection{$H + 2$~jet production, processes 271--274}
...
\subsection{$H + 3$~jet production, processes 275, 276}
...
The calculation of process {\tt 280} may be performed
at NLO using the Frixione algorithm~\cite{Frixione:1998jh}.  Note
however that the contribution in which a jet fragments into a photon
is not included, so these processes are currently of limited utility.
...
The QCD renormalisation, factorisation and fragmentation scale can be set to $ a\times m_{\gamma\gamma}$ by 
setting {\tt dynamic\_scale} equal to {\tt .true. } $a$ is then set by the value of  {\tt scale}.  Note that other dynamic scales for this process 
are not implemented. 
...
The QCD renormalisation, factorisation and fragmentation scale can be set to $ a\times \sqrt{m_{V}^2+p^2_{T,\gamma}}$ by 
setting {\tt dynamic\_scale} equal to {\tt .true. } $a$ is then set by the value of  {\tt scale}.  Note that other dynamic scales for this process 
are not implemented. 
...
When {\tt removebr} is true in process {\tt 300}, the $Z$ boson does not decay.
...
\subsection{$W+Q{\bar Q}$ production, processes 420--427}
\label{subsec:wbbfilter}
These processes represent the production of a $W$ boson and one or more jets,
at least one of which is a $b$-quark. These processes all use the same matrix
...
\hline
{\tt 420}/{\tt 425} & $(b)$ or $(\overline b)$ & + ($b,\overline b$) or ($b,j$) or ($\overline b,j$) \\
{\tt 421}/{\tt 426} & $(B)$ & + ($B,j$) \\
{\tt 422}/{\tt 427} & $(b,j)$ or $(\overline b,j)$ & + ($b,\overline b,j$) \\
\hline
\end{tabular}
\caption{The different final states allowed in the calculation of processes 420--427. A jet containing
