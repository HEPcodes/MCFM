...
\vspace*{2cm}
{\it \small Updated: April 26th, 2014}
\end{center}
...
It is also possible to compile all of {\tt MCFM} as a library.
...
\ref{subsec:wt} below.
\item {\tt frag}.
Processes 280, 285-287, 290, 295, 300-302, 305-307,  820-823 only, see sections~\ref{subsec:gamgam}, \ref{subsec:wgamma} and
...
{\tt sqrt(M\pow 2+ptj1\pow 2)} & $M^2 + \vec{p_T}_{j_1}^2$ & $M=$~mass(3+4), leading $p_T$ jet, $j_1$ \\
{\tt sqrt(M\pow 2+sumptj\pow 2)} & $M^2 + \sum \vec{p_T}_{j}^2$ & $M=$~mass(3+4), sum over all jets \\
{\tt sqrt(m(34)\pow 2+sumptj\pow 2)} & $(p_3+p_4)^2 + \sum \vec{p_T}_{j}^2$ & sum over all jets \\
{\tt pt(photon)} & $\vec{p_T}_\gamma^2$ & \\
{\tt HT} & $\sum_{i=1}^n {p_T}_i$ & $n$ final state particles \\
 &  & (partons, not jets) \\
\hline 
\hline\end{tabular}
\end{center}
...
or `{\tt GdRG\_\_LO}' for the LO and `{\tt GdRG\_NLO}' for the NLO sets of ref.~\cite{GehrmannDeRidder:1998ba}.  
...
This (currently dummy) subroutine that gets called after MCFM has written its comments to                                                    one of the output files. It allows the user to write their own comments                                                         
to that same file.
...
are included. This process may be calculated to NLO.

Note that, for processes 81 and 86, alternative final states may also be
computed by appending additional characters to {\tt NPROC} as described
in Section~\ref{specifyingZdecays}.
...
\subsection{$H \to W^+W^-$ production, processes 123-127}
...
Process {\tt 127}  gives the result for the square of the box diagrams alone, i.e. the process
$gg \to W^+W^- \to \nu_e e^+ e^- \bar{\nu_e}$.
...
\label{specifyingWdecays}
As described above, these processes refer to a final state 
$\nu_e e^+ e^- \bar{\nu_e}$.  It is however possible to specify a final state
that corresponds to a different set of $W$ boson decays.  This is achieved
...
\end{tabular}
\end{center}
The labelling of the particles in the output is best
understood by example.  Setting {\tt nproc=127.ENMN} corresponds to the
process $gg \to W^+W^- \to \nu_e(p_3) e^+(p_4) \mu^-(p_5) \bar\nu_\mu(p_6)$.
Note that the default process corresponds to the string {\tt ENEN} so that,
for instance {\tt nproc=127.ENMN} is entirely equivalent to
{\tt nproc=127}.
...
\subsection{$\gamma\gamma$ production, processes 285}
\label{subsec:gamgam}
...
Frixione algorithm~\cite{Frixione:1998jh} or standard isolation.  This process also includes
the one-loop gluon-gluon contribution as given in
ref.~\cite{Bern:2002jx}.  The production of a photon via parton fragmentation is included in the NLO calculation and 
...
\subsection{$\gamma\gamma+$ jet production, processes 286}
\label{subsec:gamgamjet}
[{\it For more details on this calculation, please refer to 1403.2641}]

Process {\tt 286} represents the production of a pair of real photons in association 
with a jet.
Since this process includes two real photons and a jet, the cross section diverges
when one of the photons (or the jet) is very soft or in the direction of the beam.
In addition it diverges when the photon and the jet are collinear.
Thus in order to produce sensible results, the input file must supply values for 
{\tt ptmin\_photon}, {\tt ptmin\_jet} and {\tt etamax\_photon}, {\tt etamax\_jet},
as well as the photon--jet separation, {\tt R(photon,jet)\_min}. This will ensure that
the cross section is well-defined.

The calculation of process {\tt 286} may be performed at NLO using either the
Frixione algorithm~\cite{Frixione:1998jh} or standard isolation.  
The production of a photon via parton fragmentation is included in the NLO calculation and 
can be run separately by using the {\tt frag} option in {\tt part}. This option includes the contributions from the integrated 
photon dipole subtraction terms and the LO QCD matrix element multiplied by the fragmentation function.  
...
The calculation of process {\tt 287} may be performed at NLO using either the
...
\item
 ...
 Phys.\ Rev.\ D {\bf 87}, no. 11, 114006 (2013)
  [arXiv:1302.3856 [hep-ph]].
\item 
  J.~M.~Campbell, R.~K.~Ellis and C.~Williams,
  {\it ``Bounding the Higgs width at the LHC using full analytic results for $gg -> e^- e^+ \mu^- \mu^+$,''} \\
  JHEP {\bf 1404}, 060 (2014)
  [arXiv:1311.3589 [hep-ph]].
\item 
  J.~M.~Campbell, R.~K.~Ellis and C.~Williams,
  {\it ``Bounding the Higgs width at the LHC: complementary results from $H \to WW$,''} \\
  Phys.\ Rev.\ D {\bf 89}, 053011 (2014)
  [arXiv:1312.1628 [hep-ph]].
\item 
  J.~M.~Campbell and C.~Williams,
  {\it ``Triphoton production at hadron colliders,''} \\
  arXiv:1403.2641 [hep-ph].
\end{itemize}
...
\hline 
81 & $ Z^0(\to e^-(p_{3})+e^+(p_{4})) + Z^0(\to \mu^-(p_{5})+\mu^+(p_{6}))$   & NLO \\
...
116 & $ H(\to Z^0(e^-(p_{3})+e^+(p_{4})) + Z^0(\mu^-(p_{5})+\mu^+(p_{6}))$   & NLO \\
117 & $ H(\to Z^0(3\times(\nu(p_{3})+\bar{\nu}(p_{4})))+ Z^0(\mu^-(p_{5})+\mu^+(p_{6}))$   & NLO \\
...
\hline 
128 & $ H(\to Z^0(e^-(p3)+e^+(p4)) + Z^0(\mu^-(p5)+\mu^+(p6))$ [top, bottom loops, exact]& LO \\
129 & $ H(\to Z^0(e^-(p3)+e^+(p4)) + Z^0(\mu^-(p5)+\mu^+(p6))$ [only H, gg$\rightarrow$ZZ intf.] & LO \\
130 & $ H(\to Z^0(e^-(p3)+e^+(p4)) + Z^0(\mu^-(p5)+\mu^+(p6))$ [$|H|^2$ and H,gg$\rightarrow$ZZ intf.]& LO \\
131 & $ Z^0(e^-(p3)+e^+(p4)) + Z^0(\mu^-(p5)+\mu^+(p6))$ [gg only, (H + gg$\rightarrow$ZZ) squared]& LO \\
132 & $ Z^0(e^-(p3)+e^+(p4)) + Z^0(\mu^-(p5)+\mu^+(p6))$ [(gg$\rightarrow$ZZ) squared]& LO \\
\hline 
...
\hline 
133 & $ H(\to Z^0(e^-(p3)+e^+(p4)) + Z^0(\mu^-(p5)+\mu^+(p6) + f(p7))$ [intf,no $p_7$ cut]& LO \\
\hline 
...
286 & $ f(p_{1})+f(p_{2})\to  \gamma(p_{3})+\gamma(p_{4})+f(p_{5})$   & NLO+F \\
...
529 & $ Z^0(\to e^-(p_3)+e^+(p_4))+t(p5)+\bar{t}(p_6)  $ & LO \\
...
\section{Version 6.8 changelog}
\label{changelog6.8}
\begin{itemize}
\item Added new process for diphoton+jet production ({\tt nproc=286})
\item Fixed error in triphoton production ({\tt nproc=287})
\item Included identical-particle interference effects for $ZZ$ final states,
{\tt nproc=81,86} and {\tt nproc=128,129,130,131,132}; extended {\tt nproc=90} to NLO
\item Included effect of $WW$/$ZZ$ interference in $2\ell2\nu$ final states,
{\tt nproc=1281, 1311, 1321} and  {\tt nproc=1282, 1312, 1322}
\item Added new method for selecting $Z$ boson decays in $ZZ$ final states,
{\tt nproc=81,86} and {\tt nproc=128,129,130,131,132}; also, additional
leptonic $W$ boson decay modes in $WW$ final states, {\tt nproc=123,124,125,126,127}
\item Included implementation of HTO form of propagator for select Higgs processes,
{\tt nproc=123}--{\tt 126}, {\tt nproc=128}--{\tt 131}, {\tt nproc=1281, 1311}
and  {\tt nproc=1282, 1312}
\item Added specific histogram routines for more processes and enabled additional
dynamic scale choices
\item Fixed bug in implementation of subleading lepton transverse momentum
and rapidity cuts
\item Updated froot.c for compatibility with most modern compilers
\end{itemize}
