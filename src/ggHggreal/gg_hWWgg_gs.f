...
*                                                                      *
*  f(-p1)+f(-p2) --> H + parton(p7) + parton(p8) + parton(p9)          *
*                    |                                                 *
...
c--- (nflav-1) and also the identical one with symmetry factor 1/2
     ...
     . +msq_struc67_2v(igg_ba,m,0))*sub67_2v)/2d0
      ...
      msq(5,j,k)=msq(5,j,k)+(aveqg/avegg)*nfactor/dfloat(nflav)*f4q*(
     ...
     . +msq_struc15_2v(igg_sym,0,k))*sub15_2v)
      ...
      msq(2,j,k)=msq(2,j,k)+dfloat(nflav-1)*f4q*(
     .  msq27_1(m,2))*sub27_1(qg)

      msq(4,j,k)=msq(4,j,k)+dfloat(nflav-1)*f4q*(
...
****************************************************************
     ...
     .  +msq_struc25_1(iqq_i,m,-1))*sub25_1(qg)
      msq(3,j,k)=msq(3,j,k)+(aveqg/avegg)*half/dfloat(nflav)*f4q*(
...
c--- (nflav-1) and also the identical one with symmetry factor 1/2
     ...
     . +msq_struc67_2v(igg_sym,0,n))*sub67_2v)/2d0
      ...
      msq(6,j,k)=msq(6,j,k)+(aveqg/avegg)*nfactor/dfloat(nflav)*f4q*(
...
c---  iqq_b = iqq_a (5 <--> 6) and we want qQ->Qq and not qQ->qQ
      msq(1,j,k)=msq(1,j,k)+2d0*dfloat(nflav-1)*f4q*(
...
****************************************************************
     ...
     .  msq17_2(1,n))*sub17_2(qg)
      msq(3,j,k)=msq(3,j,k)+half/dfloat(nflav-1)*f4q*(
...
c--- is artificially stored in msq_struc(iqr,0,0)
     ...
     .  +msq_struc15_2(iqr,0,0))*sub15_2(qg)
      msq(4,j,k)=msq(4,j,k)+(aveqg/avegg)*half/dfloat(nflav)*f4q*(
...
**********************************
     ...
     . msq_struc17_5v(igg_ba,j,k),msq_struc17_5v(igg_sym,j,k),sub17_5v)
      msq(9,j,k)=msq(9,j,k)+2d0*f2q*sub2q3g_iffi(
     ...
     . msq_struc17_6v(igg_ab,j,k),msq_struc17_6v(igg_sym,j,k),sub17_6v)
      msq(10,j,k)=msq(10,j,k)+2d0*f2q*sub2q3g_iffi(
     ...
     . msq_struc27_5v(igg_ab,j,k),msq_struc27_5v(igg_sym,j,k),sub27_5v)
      msq(8,j,k)=msq(8,j,k)+2d0*f2q*sub2q3g_iffi(
...
c--- f0q subtractions above
      msq(21,j,k)=msq(21,j,k)+2d0*f2q*dfloat(nflav)*(
     ...
     . +msq_struc56_7v(igggg_c,j,k))*sub56_7v)/2d0
      ...
      msq(11,j,k)=msq(11,j,k)+2d0*f2q*dfloat(nflav)*(
     ...
     . +msq_struc56_1v(igggg_c,j,k))*sub56_1v)/2d0
      ...
      msq(14,j,k)=msq(14,j,k)+2d0*f2q*dfloat(nflav)*(
