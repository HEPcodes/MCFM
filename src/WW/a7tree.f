...
c---u(2)+dbar(1)-->nu(3)+e^+(3)+b(5)+bbar(6)+g(7)
     ...
     . /(za(j1,j7)*s(j3,j4)*s(j5,j6)*t134)
      ...
      double complex function A7treeb(j1,j2,j3,j4,j5,j6,j7,za,zb)
...
c---u(2)+dbar(1)-->nu(3)+e^+(3)+b(5)+bbar(6)+g(7)
      ...
      A7treeb=(-za(j3,j6)*zb(j4,j5)
     . *(z2(j1,j5,j6,j2)*za(j2,j1)+z2(j1,j5,j6,j7)*za(j7,j1))
     . +za(j1,j3)*z2(j1,j2,j7,j4)*z2(j6,j3,j4,j5)
     . -za(j1,j6)*z2(j1,j2,j7,j5)*z2(j3,j5,j6,j4))
     . /(za(j1,j7)*za(j7,j2)*s(j3,j4)*s(j5,j6)*t127)
      return 
      end
