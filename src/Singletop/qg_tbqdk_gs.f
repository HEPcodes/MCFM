...
c--- subtractions for gg
     ...
     .                   +msq16_5(2,k)+msq16_5(1,k))*sub16_5(qg)*corrL
       msq(2,j,k)=2d0*tr*(msq26_5(j,4)+msq26_5(j,3)
     .                   +msq26_5(j,2)+msq26_5(j,1))*sub26_5(qg)*corrL
       msq(3,j,k)=2d0*tr*(msq15_6(-4,k)+msq15_6(-3,k)
...
c     . 2d0*dot(p,1,3)/dot(p,1,6)-mt**2/dot(p,3,6))/dot(p,3,6)
c      e14=xn/2d0*gsq*dummyv(0,4)*(
c     . 2d0*dot(p,1,4)/dot(p,1,6)-mb**2/dot(p,4,6))/dot(p,4,6)
c      e34=-1d0/2d0/xn*gsq*dummyv(0,4)*(
c     . 2d0*dot(p,3,4)/dot(p,3,6)/dot(p,4,6)
...
c      e15=cf*gsq*(2d0*dot(p,1,5))
c     . /dot(p,1,6)/dot(p,5,6)*dummyv(4,0)
c      e23=xn/2d0*gsq*dummyv(4,0)*(
c     . 2d0*dot(p,2,3)/dot(p,2,6)-mt**2/dot(p,3,6))/dot(p,3,6)
c      e24=xn/2d0*gsq*dummyv(4,0)*(
c     . 2d0*dot(p,2,4)/dot(p,2,6)-mb**2/dot(p,4,6))/dot(p,4,6)
c      e34=-1d0/2d0/xn*gsq*dummyv(4,0)*(
c     . 2d0*dot(p,3,4)/dot(p,3,6)/dot(p,4,6)
c     . -mb**2/dot(p,4,6)**2-mt**2/dot(p,3,6)**2)
c      write(6,*) '(4,0)'
