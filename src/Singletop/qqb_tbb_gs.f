...
c     . bq_tpq,donothing_gvec)
     ...
     .     .or.(j .lt. 0) .and. (k .gt. 0)) then
         ...
         msq(3,j,k)=2d0*cf*sub35_4(qq)*msq35_4(j,k)
         msq(4,j,k)=2d0*cf*sub45_3(qq)*msq45_3(j,k)
...
c--- checked 15,25,45 singularities, soft singularity looks ok      
        ...
        msq(4,j,k)=2d0*cf*sub35_2(qq)*msq35_2(j,k)
c--- checked
      elseif ((j .eq. ib) .and. (k .eq. 0)) then
         msq(10,j,k)=2d0*tr*sub25_1(qg)*(msq25_1(j,+1)+msq25_1(j,+2)
     &   +msq25_1(j,+3)+msq25_1(j,+4)+msq25_1(j,+5))
