...
************************************************************************
     ...
     . mc2,ms2,qsq,s,t,u,xsn,xsd,xs
      double complex trc,trg,trs,trsgc,zppp,Bppp,old
     ...
     &   3Xc/((cDs**2-mc2*ms2)*gDs)/2.0d+0+Bppp
     ...
     &   Dg**5)/(mc*ms2*cDg**2*gDs**2*(gDs+cDg))/4.0d+0+Bppp
     ...
     &   5)/(mc*ms2*cDg**2*gDs**2)/4.0d+0+Bppp
     ...
     &   *3)/((cDs-mc*ms)*(cDs+mc*ms)*(gDs+cDg))/2.0d+0+Bppp
     ...
     &   *cDs+mc*(ms-ms*xs**2))*gDs)/(ms*(xs**2-1)*cDg*gDs)/4.0d+0+Bppp
     ...
     &   Ds)/(mc*gDs**2)
      ...
      Bppp = ls*ms2*(-2*trc*gDs+trsgc+mc2*trg)/(mc*(2*gDs+ms2))/2.0d+0+
     &   Bppp
