...
************************************************************************
     ...
     . mc2,ms2,qsq,s,t,u,xsn,xsd,xs
      double complex trc,trg,trs,trsgc,zmmm,Bmmm,old
     ...
     &   *lVc/(ms*cDg**2*(cDs-mc*ms)*(cDs+mc*ms)*gDs)/4.0d+0+Bmmm
     ...
     &   (8*trs-trg+10*trc)))/(ms*cDg**2*gDs**2*(2*gDs+ms2))/1.2d+1+Bmm
     &   m
     ...
     &   (trs+trc)))/(ms*cDg**2*gDs**2*(gDs+cDg))/4.0d+0+Bmmm
     ...
     &   +mc2))/((cDs**2-mc2*ms2)*(gDs+cDg))/2.0d+0+Bmmm
     ...
     &   *cDs+mc*(ms-ms*xs**2))*gDs)/(mc*(xs**2-1)*cDg*gDs)/4.0d+0+Bmmm

      Bmmm = ls*(ms*(2*gDs+ms2)*(-2*trs*gDs+4*trg*gDs+3*ms2*trg)-ms*trs
     &   gc*(4*gDs+ms2))/(2*gDs+ms2)**2/2.0d+0+Bmmm
