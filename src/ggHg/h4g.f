...
C---hep-ph/9903330 Eq(11)
     ...
     . -zb(i2(j),i4(j))*s234)
C      if (k.eq.1) amppp(j)=temp
C      if (k.eq.2) apmpp(j)=temp
C      if (k.eq.3) appmp(j)=temp
...
C---determine axxxx(3) using sub-cyclic identity
      if (j .eq.2) then 
         ...
         if (k.eq.4) apppm(2)=-apppm(1)-apppm(3)
      endif
...
C            if (k.eq.1) ammpp(j)=temp
C            if (k.eq.2) ampmp(j)=temp
C            if (k.eq.3) amppm(j)=temp
C            if (k.eq.4) apmmp(j)=temp
