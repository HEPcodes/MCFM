...
c      write(*,*) 'approx ',msq(0,0)
      
c--- exact matrix element for mf=175
      origmbsq=mbsq
      mbsq=175d0**2
      gg=cdabs(mbsq/s12*(4d0-w2(s12)*(1d0-4d0*mbsq/s12)))**2
      gg=aw*as**2*V/(8d0*pi)*(s12/wmass)**2*gg
      msq(0,0)=avegg*prop*gg*hdecay
      mbsq=origmbsq
c      write(*,*) ' exact ',msq(0,0)

c--- pseudoscalar Higgs coupling to b with tan(beta)=30
      origmbsq=mbsq
      mbsq=21.344d0
      gg=cdabs(-mbsq/2d0/s12*w2(s12))**2
      gg=gg*aw*as**2*V/(8d0*pi)*(s12/wmass)**2
      gg=gg*3d0*30d0**2
      msq(0,0)=avegg*prop*gg*hdecay
      mbsq=origmbsq
