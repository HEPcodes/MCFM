...
************************************************************************
      ...
      integer countlept,leptindex(mxpart),countgamm,countnu,
     ...
     & etalept,mll,jetpt,jetrap,mnul,etabuffer
      logical hwwjetcuts,gencuts_Zt,gencuts_WZjj
...
c -- CMS FCNC cuts for Z_tdkj
      if ( case .eq. 'Z_tdkj' .and. runstring(1:3) .eq. 'CMS' ) then
         gencuts=gencuts_Zt(pjet,njets)
         return
      endif

      if ( case .eq. 'WpmZjj' .and. runstring(1:3) .eq. 'CMS' ) then
         gencuts=gencuts_WZjj(pjet,njets)
         return
      endif
...
c -- mll lepton cut for Ztjet production
     ...
     .        -pjet(leptindex(1),2)*pjet(leptindex(2),2)
     .        -pjet(leptindex(1),3)*pjet(leptindex(2),3)))
         if ( abs(mll-zmass) .ge. mllcut) then
            gencuts=.true.
            return
         endif
      endif
     ...
     .        -pjet(nuindex(1),3)*pjet(leptindex(3),3)))
         if ( abs(mnul-wmass) .ge. mnulcut) then
            gencuts=.true.
            return
         endif
      endif
