      REAL*8 FUNCTION SDUB_EMVESSBG(P1, P2, P3, P4, P5, P6, P7)
C  
C FUNCTION GENERATED BY MADGRAPH
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C AND HELICITIES
C FOR THE POINT IN PHASE SPACE P1,P2,P3,P4,...
C  
C FOR PROCESS : d u~  -> e- ve~ s s~ g  
C  
      IMPLICIT NONE
C  
C CONSTANTS
C  
      INTEGER    NEXTERNAL,   NCOMB                     
      PARAMETER (NEXTERNAL=7, NCOMB=128)
C  
C ARGUMENTS 
C  
      REAL*8 P1(0:3),P2(0:3),P3(0:3),P4(0:3),P5(0:3),P6(0:3),P7(0:3)             
C  
C LOCAL VARIABLES 
C  
      INTEGER NHEL(NEXTERNAL,NCOMB),NTRY                                         
      REAL*8 T
      REAL*8 DUB_EMVESSBG                                                        
      INTEGER IHEL
      LOGICAL GOODHEL(NCOMB)
      DATA GOODHEL/NCOMB*.FALSE./
      DATA NTRY/0/
      DATA (NHEL(IHEL,  1),IHEL=1,7) / -1, -1, -1, -1, -1, -1, -1/
      DATA (NHEL(IHEL,  2),IHEL=1,7) / -1, -1, -1, -1, -1, -1,  1/
      DATA (NHEL(IHEL,  3),IHEL=1,7) / -1, -1, -1, -1, -1,  1, -1/
      DATA (NHEL(IHEL,  4),IHEL=1,7) / -1, -1, -1, -1, -1,  1,  1/
      DATA (NHEL(IHEL,  5),IHEL=1,7) / -1, -1, -1, -1,  1, -1, -1/
      DATA (NHEL(IHEL,  6),IHEL=1,7) / -1, -1, -1, -1,  1, -1,  1/
      DATA (NHEL(IHEL,  7),IHEL=1,7) / -1, -1, -1, -1,  1,  1, -1/
      DATA (NHEL(IHEL,  8),IHEL=1,7) / -1, -1, -1, -1,  1,  1,  1/
      DATA (NHEL(IHEL,  9),IHEL=1,7) / -1, -1, -1,  1, -1, -1, -1/
      DATA (NHEL(IHEL, 10),IHEL=1,7) / -1, -1, -1,  1, -1, -1,  1/
      DATA (NHEL(IHEL, 11),IHEL=1,7) / -1, -1, -1,  1, -1,  1, -1/
      DATA (NHEL(IHEL, 12),IHEL=1,7) / -1, -1, -1,  1, -1,  1,  1/
      DATA (NHEL(IHEL, 13),IHEL=1,7) / -1, -1, -1,  1,  1, -1, -1/
      DATA (NHEL(IHEL, 14),IHEL=1,7) / -1, -1, -1,  1,  1, -1,  1/
      DATA (NHEL(IHEL, 15),IHEL=1,7) / -1, -1, -1,  1,  1,  1, -1/
      DATA (NHEL(IHEL, 16),IHEL=1,7) / -1, -1, -1,  1,  1,  1,  1/
      DATA (NHEL(IHEL, 17),IHEL=1,7) / -1, -1,  1, -1, -1, -1, -1/
      DATA (NHEL(IHEL, 18),IHEL=1,7) / -1, -1,  1, -1, -1, -1,  1/
      DATA (NHEL(IHEL, 19),IHEL=1,7) / -1, -1,  1, -1, -1,  1, -1/
      DATA (NHEL(IHEL, 20),IHEL=1,7) / -1, -1,  1, -1, -1,  1,  1/
      DATA (NHEL(IHEL, 21),IHEL=1,7) / -1, -1,  1, -1,  1, -1, -1/
      DATA (NHEL(IHEL, 22),IHEL=1,7) / -1, -1,  1, -1,  1, -1,  1/
      DATA (NHEL(IHEL, 23),IHEL=1,7) / -1, -1,  1, -1,  1,  1, -1/
      DATA (NHEL(IHEL, 24),IHEL=1,7) / -1, -1,  1, -1,  1,  1,  1/
      DATA (NHEL(IHEL, 25),IHEL=1,7) / -1, -1,  1,  1, -1, -1, -1/
      DATA (NHEL(IHEL, 26),IHEL=1,7) / -1, -1,  1,  1, -1, -1,  1/
      DATA (NHEL(IHEL, 27),IHEL=1,7) / -1, -1,  1,  1, -1,  1, -1/
      DATA (NHEL(IHEL, 28),IHEL=1,7) / -1, -1,  1,  1, -1,  1,  1/
      DATA (NHEL(IHEL, 29),IHEL=1,7) / -1, -1,  1,  1,  1, -1, -1/
      DATA (NHEL(IHEL, 30),IHEL=1,7) / -1, -1,  1,  1,  1, -1,  1/
      DATA (NHEL(IHEL, 31),IHEL=1,7) / -1, -1,  1,  1,  1,  1, -1/
      DATA (NHEL(IHEL, 32),IHEL=1,7) / -1, -1,  1,  1,  1,  1,  1/
      DATA (NHEL(IHEL, 33),IHEL=1,7) / -1,  1, -1, -1, -1, -1, -1/
      DATA (NHEL(IHEL, 34),IHEL=1,7) / -1,  1, -1, -1, -1, -1,  1/
      DATA (NHEL(IHEL, 35),IHEL=1,7) / -1,  1, -1, -1, -1,  1, -1/
      DATA (NHEL(IHEL, 36),IHEL=1,7) / -1,  1, -1, -1, -1,  1,  1/
      DATA (NHEL(IHEL, 37),IHEL=1,7) / -1,  1, -1, -1,  1, -1, -1/
      DATA (NHEL(IHEL, 38),IHEL=1,7) / -1,  1, -1, -1,  1, -1,  1/
      DATA (NHEL(IHEL, 39),IHEL=1,7) / -1,  1, -1, -1,  1,  1, -1/
      DATA (NHEL(IHEL, 40),IHEL=1,7) / -1,  1, -1, -1,  1,  1,  1/
      DATA (NHEL(IHEL, 41),IHEL=1,7) / -1,  1, -1,  1, -1, -1, -1/
      DATA (NHEL(IHEL, 42),IHEL=1,7) / -1,  1, -1,  1, -1, -1,  1/
      DATA (NHEL(IHEL, 43),IHEL=1,7) / -1,  1, -1,  1, -1,  1, -1/
      DATA (NHEL(IHEL, 44),IHEL=1,7) / -1,  1, -1,  1, -1,  1,  1/
      DATA (NHEL(IHEL, 45),IHEL=1,7) / -1,  1, -1,  1,  1, -1, -1/
      DATA (NHEL(IHEL, 46),IHEL=1,7) / -1,  1, -1,  1,  1, -1,  1/
      DATA (NHEL(IHEL, 47),IHEL=1,7) / -1,  1, -1,  1,  1,  1, -1/
      DATA (NHEL(IHEL, 48),IHEL=1,7) / -1,  1, -1,  1,  1,  1,  1/
      DATA (NHEL(IHEL, 49),IHEL=1,7) / -1,  1,  1, -1, -1, -1, -1/
      DATA (NHEL(IHEL, 50),IHEL=1,7) / -1,  1,  1, -1, -1, -1,  1/
      DATA (NHEL(IHEL, 51),IHEL=1,7) / -1,  1,  1, -1, -1,  1, -1/
      DATA (NHEL(IHEL, 52),IHEL=1,7) / -1,  1,  1, -1, -1,  1,  1/
      DATA (NHEL(IHEL, 53),IHEL=1,7) / -1,  1,  1, -1,  1, -1, -1/
      DATA (NHEL(IHEL, 54),IHEL=1,7) / -1,  1,  1, -1,  1, -1,  1/
      DATA (NHEL(IHEL, 55),IHEL=1,7) / -1,  1,  1, -1,  1,  1, -1/
      DATA (NHEL(IHEL, 56),IHEL=1,7) / -1,  1,  1, -1,  1,  1,  1/
      DATA (NHEL(IHEL, 57),IHEL=1,7) / -1,  1,  1,  1, -1, -1, -1/
      DATA (NHEL(IHEL, 58),IHEL=1,7) / -1,  1,  1,  1, -1, -1,  1/
      DATA (NHEL(IHEL, 59),IHEL=1,7) / -1,  1,  1,  1, -1,  1, -1/
      DATA (NHEL(IHEL, 60),IHEL=1,7) / -1,  1,  1,  1, -1,  1,  1/
      DATA (NHEL(IHEL, 61),IHEL=1,7) / -1,  1,  1,  1,  1, -1, -1/
      DATA (NHEL(IHEL, 62),IHEL=1,7) / -1,  1,  1,  1,  1, -1,  1/
      DATA (NHEL(IHEL, 63),IHEL=1,7) / -1,  1,  1,  1,  1,  1, -1/
      DATA (NHEL(IHEL, 64),IHEL=1,7) / -1,  1,  1,  1,  1,  1,  1/
      DATA (NHEL(IHEL, 65),IHEL=1,7) /  1, -1, -1, -1, -1, -1, -1/
      DATA (NHEL(IHEL, 66),IHEL=1,7) /  1, -1, -1, -1, -1, -1,  1/
      DATA (NHEL(IHEL, 67),IHEL=1,7) /  1, -1, -1, -1, -1,  1, -1/
      DATA (NHEL(IHEL, 68),IHEL=1,7) /  1, -1, -1, -1, -1,  1,  1/
      DATA (NHEL(IHEL, 69),IHEL=1,7) /  1, -1, -1, -1,  1, -1, -1/
      DATA (NHEL(IHEL, 70),IHEL=1,7) /  1, -1, -1, -1,  1, -1,  1/
      DATA (NHEL(IHEL, 71),IHEL=1,7) /  1, -1, -1, -1,  1,  1, -1/
      DATA (NHEL(IHEL, 72),IHEL=1,7) /  1, -1, -1, -1,  1,  1,  1/
      DATA (NHEL(IHEL, 73),IHEL=1,7) /  1, -1, -1,  1, -1, -1, -1/
      DATA (NHEL(IHEL, 74),IHEL=1,7) /  1, -1, -1,  1, -1, -1,  1/
      DATA (NHEL(IHEL, 75),IHEL=1,7) /  1, -1, -1,  1, -1,  1, -1/
      DATA (NHEL(IHEL, 76),IHEL=1,7) /  1, -1, -1,  1, -1,  1,  1/
      DATA (NHEL(IHEL, 77),IHEL=1,7) /  1, -1, -1,  1,  1, -1, -1/
      DATA (NHEL(IHEL, 78),IHEL=1,7) /  1, -1, -1,  1,  1, -1,  1/
      DATA (NHEL(IHEL, 79),IHEL=1,7) /  1, -1, -1,  1,  1,  1, -1/
      DATA (NHEL(IHEL, 80),IHEL=1,7) /  1, -1, -1,  1,  1,  1,  1/
      DATA (NHEL(IHEL, 81),IHEL=1,7) /  1, -1,  1, -1, -1, -1, -1/
      DATA (NHEL(IHEL, 82),IHEL=1,7) /  1, -1,  1, -1, -1, -1,  1/
      DATA (NHEL(IHEL, 83),IHEL=1,7) /  1, -1,  1, -1, -1,  1, -1/
      DATA (NHEL(IHEL, 84),IHEL=1,7) /  1, -1,  1, -1, -1,  1,  1/
      DATA (NHEL(IHEL, 85),IHEL=1,7) /  1, -1,  1, -1,  1, -1, -1/
      DATA (NHEL(IHEL, 86),IHEL=1,7) /  1, -1,  1, -1,  1, -1,  1/
      DATA (NHEL(IHEL, 87),IHEL=1,7) /  1, -1,  1, -1,  1,  1, -1/
      DATA (NHEL(IHEL, 88),IHEL=1,7) /  1, -1,  1, -1,  1,  1,  1/
      DATA (NHEL(IHEL, 89),IHEL=1,7) /  1, -1,  1,  1, -1, -1, -1/
      DATA (NHEL(IHEL, 90),IHEL=1,7) /  1, -1,  1,  1, -1, -1,  1/
      DATA (NHEL(IHEL, 91),IHEL=1,7) /  1, -1,  1,  1, -1,  1, -1/
      DATA (NHEL(IHEL, 92),IHEL=1,7) /  1, -1,  1,  1, -1,  1,  1/
      DATA (NHEL(IHEL, 93),IHEL=1,7) /  1, -1,  1,  1,  1, -1, -1/
      DATA (NHEL(IHEL, 94),IHEL=1,7) /  1, -1,  1,  1,  1, -1,  1/
      DATA (NHEL(IHEL, 95),IHEL=1,7) /  1, -1,  1,  1,  1,  1, -1/
      DATA (NHEL(IHEL, 96),IHEL=1,7) /  1, -1,  1,  1,  1,  1,  1/
      DATA (NHEL(IHEL, 97),IHEL=1,7) /  1,  1, -1, -1, -1, -1, -1/
      DATA (NHEL(IHEL, 98),IHEL=1,7) /  1,  1, -1, -1, -1, -1,  1/
      DATA (NHEL(IHEL, 99),IHEL=1,7) /  1,  1, -1, -1, -1,  1, -1/
      DATA (NHEL(IHEL,100),IHEL=1,7) /  1,  1, -1, -1, -1,  1,  1/
      DATA (NHEL(IHEL,101),IHEL=1,7) /  1,  1, -1, -1,  1, -1, -1/
      DATA (NHEL(IHEL,102),IHEL=1,7) /  1,  1, -1, -1,  1, -1,  1/
      DATA (NHEL(IHEL,103),IHEL=1,7) /  1,  1, -1, -1,  1,  1, -1/
      DATA (NHEL(IHEL,104),IHEL=1,7) /  1,  1, -1, -1,  1,  1,  1/
      DATA (NHEL(IHEL,105),IHEL=1,7) /  1,  1, -1,  1, -1, -1, -1/
      DATA (NHEL(IHEL,106),IHEL=1,7) /  1,  1, -1,  1, -1, -1,  1/
      DATA (NHEL(IHEL,107),IHEL=1,7) /  1,  1, -1,  1, -1,  1, -1/
      DATA (NHEL(IHEL,108),IHEL=1,7) /  1,  1, -1,  1, -1,  1,  1/
      DATA (NHEL(IHEL,109),IHEL=1,7) /  1,  1, -1,  1,  1, -1, -1/
      DATA (NHEL(IHEL,110),IHEL=1,7) /  1,  1, -1,  1,  1, -1,  1/
      DATA (NHEL(IHEL,111),IHEL=1,7) /  1,  1, -1,  1,  1,  1, -1/
      DATA (NHEL(IHEL,112),IHEL=1,7) /  1,  1, -1,  1,  1,  1,  1/
      DATA (NHEL(IHEL,113),IHEL=1,7) /  1,  1,  1, -1, -1, -1, -1/
      DATA (NHEL(IHEL,114),IHEL=1,7) /  1,  1,  1, -1, -1, -1,  1/
      DATA (NHEL(IHEL,115),IHEL=1,7) /  1,  1,  1, -1, -1,  1, -1/
      DATA (NHEL(IHEL,116),IHEL=1,7) /  1,  1,  1, -1, -1,  1,  1/
      DATA (NHEL(IHEL,117),IHEL=1,7) /  1,  1,  1, -1,  1, -1, -1/
      DATA (NHEL(IHEL,118),IHEL=1,7) /  1,  1,  1, -1,  1, -1,  1/
      DATA (NHEL(IHEL,119),IHEL=1,7) /  1,  1,  1, -1,  1,  1, -1/
      DATA (NHEL(IHEL,120),IHEL=1,7) /  1,  1,  1, -1,  1,  1,  1/
      DATA (NHEL(IHEL,121),IHEL=1,7) /  1,  1,  1,  1, -1, -1, -1/
      DATA (NHEL(IHEL,122),IHEL=1,7) /  1,  1,  1,  1, -1, -1,  1/
      DATA (NHEL(IHEL,123),IHEL=1,7) /  1,  1,  1,  1, -1,  1, -1/
      DATA (NHEL(IHEL,124),IHEL=1,7) /  1,  1,  1,  1, -1,  1,  1/
      DATA (NHEL(IHEL,125),IHEL=1,7) /  1,  1,  1,  1,  1, -1, -1/
      DATA (NHEL(IHEL,126),IHEL=1,7) /  1,  1,  1,  1,  1, -1,  1/
      DATA (NHEL(IHEL,127),IHEL=1,7) /  1,  1,  1,  1,  1,  1, -1/
      DATA (NHEL(IHEL,128),IHEL=1,7) /  1,  1,  1,  1,  1,  1,  1/
C ----------
C BEGIN CODE
C ----------
      SDUB_EMVESSBG = 0d0
      NTRY=NTRY+1
      DO IHEL=1,NCOMB
          IF (GOODHEL(IHEL) .OR. NTRY .LT. 10) THEN
             T=DUB_EMVESSBG(P1, P2, P3, P4, P5, P6, P7,NHEL(1,IHEL)) 
             SDUB_EMVESSBG = SDUB_EMVESSBG + T
              IF (T .GT. 0D0 .AND. .NOT. GOODHEL(IHEL)) THEN
                  GOODHEL(IHEL)=.TRUE.
                  WRITE(*,*) IHEL,T
              ENDIF
          ENDIF
      ENDDO
      SDUB_EMVESSBG = SDUB_EMVESSBG /  4D0 
      END
       
       
      REAL*8 FUNCTION DUB_EMVESSBG(P1, P2, P3, P4, P5, P6, P7,NHEL)
C  
C FUNCTION GENERATED BY MADGRAPH
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT IN PHASE SPACE P1,P2,P3,P4,...
C AND HELICITY NHEL(1),NHEL(2),....
C  
C FOR PROCESS : d u~  -> e- ve~ s s~ g  
C  
      IMPLICIT NONE
      include 'zerowidth.f'
C  
C CONSTANTS
C  
      INTEGER    NGRAPHS,    NEIGEN,    NEXTERNAL       
      PARAMETER (NGRAPHS= 45,NEIGEN=  2,NEXTERNAL=7)    
      REAL*8     ZERO
      PARAMETER (ZERO=0D0)
C  
C ARGUMENTS 
C  
      REAL*8 P1(0:3),P2(0:3),P3(0:3),P4(0:3),P5(0:3),P6(0:3),P7(0:3)             
      INTEGER NHEL(NEXTERNAL)                                                    
C  
C LOCAL VARIABLES 
C  
      INTEGER I,J
      REAL*8 EIGEN_VAL(NEIGEN), EIGEN_VEC(NGRAPHS,NEIGEN)
      COMPLEX*16 ZTEMP
      COMPLEX*16 AMP(NGRAPHS)
      COMPLEX*16 W1(6)  , W2(6)  , W3(6)  , W4(6)  , W5(6)        
      COMPLEX*16 W6(6)  , W7(6)  , W8(6)  , W9(6)  , W10(6)       
      COMPLEX*16 W11(6) , W12(6) , W13(6) , W14(6) , W15(6)       
      COMPLEX*16 W16(6) , W17(6) , W18(6) , W19(6) , W20(6)       
      COMPLEX*16 W21(6) , W22(6) , W23(6) , W24(6) , W25(6)       
      COMPLEX*16 W26(6) , W27(6) , W28(6) , W29(6) , W30(6)       
      COMPLEX*16 W31(6) , W32(6) , W33(6) , W34(6) , W35(6)       
      COMPLEX*16 W36(6) , W37(6) , W38(6) , W39(6) , W40(6)       
      COMPLEX*16 W41(6) , W42(6) , W43(6) , W44(6) , W45(6)       
      COMPLEX*16 W46(6) , W47(6) , W48(6) , W49(6) , W50(6)       
      COMPLEX*16 W51(6) , W52(6) , W53(6) 
C  
C GLOBAL VARIABLES
C  
      REAL*8         GW, GWWA, GWWZ
      COMMON /COUP1/ GW, GWWA, GWWZ
      REAL*8         GAL(2),GAU(2),GAD(2),GWF(2)
      COMMON /COUP2A/GAL,   GAU,   GAD,   GWF
      REAL*8         GZN(2),GZL(2),GZU(2),GZD(2),G1(2)
      COMMON /COUP2B/GZN,   GZL,   GZU,   GZD,   G1
      REAL*8         GWWH,GZZH,GHHH,GWWHH,GZZHH,GHHHH
      COMMON /COUP3/ GWWH,GZZH,GHHH,GWWHH,GZZHH,GHHHH
      COMPLEX*16     GH(2,12)
      COMMON /COUP4/ GH
      REAL*8         WMASS,WWIDTH,ZMASS,ZWIDTH
      COMMON /VMASS1/WMASS,WWIDTH,ZMASS,ZWIDTH
      REAL*8         AMASS,AWIDTH,HMASS,HWIDTH
      COMMON /VMASS2/AMASS,AWIDTH,HMASS,HWIDTH
      REAL*8           GG(2), G
      COMMON /COUPQCD/ GG,    G
      REAL*8            FMASS(12), FWIDTH(12)
      COMMON /FERMIONS/ FMASS,     FWIDTH
C  
C COLOR DATA
C  
      DATA EIGEN_VAL(1  )/       3.1999999999999993E+01 /                  
      DATA EIGEN_VEC(1  ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(2  ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(3  ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(4  ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(5  ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(6  ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(7  ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(8  ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(9  ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(10 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(11 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(12 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(13 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(14 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(15 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(16 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(17 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(18 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(19 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(20 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(21 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(22 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(23 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(24 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(25 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(26 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(27 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(28 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(29 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(30 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(31 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(32 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(33 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(34 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(35 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(36 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(37 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(38 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(39 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(40 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(41 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(42 ,1  )/  -2.0412414523193154E-01 /                  
      DATA EIGEN_VEC(43 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(44 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(45 ,1  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VAL(2  )/       2.7999999999999996E+01 /                  
      DATA EIGEN_VEC(1  ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(2  ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(3  ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(4  ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(5  ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(6  ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(7  ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(8  ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(9  ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(10 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(11 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(12 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(13 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(14 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(15 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(16 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(17 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(18 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(19 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(20 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(21 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(22 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(23 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(24 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(25 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(26 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(27 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(28 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(29 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(30 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(31 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(32 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(33 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(34 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(35 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(36 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(37 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(38 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(39 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(40 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(41 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(42 ,2  )/   0.0000000000000000E+00 /                  
      DATA EIGEN_VEC(43 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(44 ,2  )/  -2.1821789023599239E-01 /                  
      DATA EIGEN_VEC(45 ,2  )/  -2.1821789023599239E-01 /                  
C ----------
C BEGIN CODE
C ----------
      CALL IXXXXX(P1  ,FMASS(4  ),NHEL(1  ), 1,W1  )                       
      CALL OXXXXX(P2  ,FMASS(3  ),NHEL(2  ),-1,W2  )                       
      CALL OXXXXX(P3  ,FMASS(1  ),NHEL(3  ), 1,W3  )                       
      CALL IXXXXX(P4  ,FMASS(2  ),NHEL(4  ),-1,W4  )                       
      CALL OXXXXX(P5  ,FMASS(8  ),NHEL(5  ), 1,W5  )                       
      CALL IXXXXX(P6  ,FMASS(8  ),NHEL(6  ),-1,W6  )                       
      CALL VXXXXX(P7  , ZERO,NHEL(7  ), 1,W7  )                            
      CALL JIOXXX(W4  ,W3  ,GWF,WMASS,WWIDTH,W8  )                         
      CALL JIOXXX(W6  ,W5  ,GAD,AMASS,AWIDTH,W9  )                         
      CALL FVIXXX(W1  ,W7  ,GG,FMASS(4  ),FWIDTH(4  ),W10 )                
      CALL FVOXXX(W2  ,W8  ,GWF,FMASS(4  ),FWIDTH(4  ),W11 )               
      CALL IOVXXX(W10 ,W11 ,W9  ,GAD,AMP(1  ))                             
      CALL JIOXXX(W6  ,W5  ,GZD,ZMASS,ZWIDTH,W12 )                         
      CALL IOVXXX(W10 ,W11 ,W12 ,GZD,AMP(2  ))                             
      CALL FVOXXX(W2  ,W7  ,GG,FMASS(3  ),FWIDTH(3  ),W13 )                
      CALL FVOXXX(W13 ,W8  ,GWF,FMASS(4  ),FWIDTH(4  ),W14 )               
      CALL IOVXXX(W1  ,W14 ,W9  ,GAD,AMP(3  ))                             
      CALL IOVXXX(W1  ,W14 ,W12 ,GZD,AMP(4  ))                             
      CALL FVOXXX(W5  ,W7  ,GG,FMASS(8  ),FWIDTH(8  ),W15 )                
      CALL JIOXXX(W1  ,W11 ,GAD,AMASS,AWIDTH,W16 )                         
      CALL IOVXXX(W6  ,W15 ,W16 ,GAD,AMP(5  ))                             
      CALL JIOXXX(W1  ,W11 ,GZD,ZMASS,ZWIDTH,W17 )                         
      CALL IOVXXX(W6  ,W15 ,W17 ,GZD,AMP(6  ))                             
      CALL FVIXXX(W6  ,W7  ,GG,FMASS(8  ),FWIDTH(8  ),W18 )                
      CALL IOVXXX(W18 ,W5  ,W16 ,GAD,AMP(7  ))                             
      CALL IOVXXX(W18 ,W5  ,W17 ,GZD,AMP(8  ))                             
      CALL FVIXXX(W1  ,W9  ,GAD,FMASS(4  ),FWIDTH(4  ),W19 )               
      CALL IOVXXX(W19 ,W11 ,W7  ,GG,AMP(9  ))                              
      CALL FVIXXX(W1  ,W12 ,GZD,FMASS(4  ),FWIDTH(4  ),W20 )               
      CALL IOVXXX(W20 ,W11 ,W7  ,GG,AMP(10 ))                              
      CALL FVIXXX(W10 ,W8  ,GWF,FMASS(3  ),FWIDTH(3  ),W21 )               
      CALL IOVXXX(W21 ,W2  ,W9  ,GAU,AMP(11 ))                             
      CALL IOVXXX(W21 ,W2  ,W12 ,GZU,AMP(12 ))                             
      CALL FVIXXX(W1  ,W8  ,GWF,FMASS(3  ),FWIDTH(3  ),W22 )               
      CALL IOVXXX(W22 ,W13 ,W9  ,GAU,AMP(13 ))                             
      CALL IOVXXX(W22 ,W13 ,W12 ,GZU,AMP(14 ))                             
      CALL JIOXXX(W22 ,W2  ,GAU,AMASS,AWIDTH,W23 )                         
      CALL IOVXXX(W6  ,W15 ,W23 ,GAD,AMP(15 ))                             
      CALL JIOXXX(W22 ,W2  ,GZU,ZMASS,ZWIDTH,W24 )                         
      CALL IOVXXX(W6  ,W15 ,W24 ,GZD,AMP(16 ))                             
      CALL IOVXXX(W18 ,W5  ,W23 ,GAD,AMP(17 ))                             
      CALL IOVXXX(W18 ,W5  ,W24 ,GZD,AMP(18 ))                             
      CALL FVOXXX(W2  ,W9  ,GAU,FMASS(3  ),FWIDTH(3  ),W25 )               
      CALL IOVXXX(W22 ,W25 ,W7  ,GG,AMP(19 ))                              
      CALL FVOXXX(W2  ,W12 ,GZU,FMASS(3  ),FWIDTH(3  ),W26 )               
      CALL IOVXXX(W22 ,W26 ,W7  ,GG,AMP(20 ))                              
      CALL JIOXXX(W10 ,W2  ,GWF,WMASS,WWIDTH,W27 )                         
      CALL FVIXXX(W4  ,W27 ,GWF,FMASS(1  ),FWIDTH(1  ),W28 )               
      CALL IOVXXX(W28 ,W3  ,W9  ,GAL,AMP(21 ))                             
      CALL IOVXXX(W28 ,W3  ,W12 ,GZL,AMP(22 ))                             
      CALL JIOXXX(W1  ,W13 ,GWF,WMASS,WWIDTH,W29 )                         
      CALL FVIXXX(W4  ,W29 ,GWF,FMASS(1  ),FWIDTH(1  ),W30 )               
      CALL IOVXXX(W30 ,W3  ,W9  ,GAL,AMP(23 ))                             
      CALL IOVXXX(W30 ,W3  ,W12 ,GZL,AMP(24 ))                             
      CALL JIOXXX(W1  ,W2  ,GWF,WMASS,WWIDTH,W31 )                         
      CALL FVIXXX(W4  ,W31 ,GWF,FMASS(1  ),FWIDTH(1  ),W32 )               
      CALL JIOXXX(W32 ,W3  ,GAL,AMASS,AWIDTH,W33 )                         
      CALL FVIXXX(W6  ,W33 ,GAD,FMASS(8  ),FWIDTH(8  ),W34 )               
      CALL IOVXXX(W34 ,W5  ,W7  ,GG,AMP(25 ))                              
      CALL JIOXXX(W32 ,W3  ,GZL,ZMASS,ZWIDTH,W35 )                         
      CALL FVIXXX(W6  ,W35 ,GZD,FMASS(8  ),FWIDTH(8  ),W36 )               
      CALL IOVXXX(W36 ,W5  ,W7  ,GG,AMP(26 ))                              
      CALL FVOXXX(W5  ,W33 ,GAD,FMASS(8  ),FWIDTH(8  ),W37 )               
      CALL IOVXXX(W6  ,W37 ,W7  ,GG,AMP(27 ))                              
      CALL FVOXXX(W5  ,W35 ,GZD,FMASS(8  ),FWIDTH(8  ),W38 )               
      CALL IOVXXX(W6  ,W38 ,W7  ,GG,AMP(28 ))                              
      CALL FVOXXX(W3  ,W27 ,GWF,FMASS(2  ),FWIDTH(2  ),W39 )               
      CALL IOVXXX(W4  ,W39 ,W12 ,GZN,AMP(29 ))                             
      CALL FVOXXX(W3  ,W29 ,GWF,FMASS(2  ),FWIDTH(2  ),W40 )               
      CALL IOVXXX(W4  ,W40 ,W12 ,GZN,AMP(30 ))                             
      CALL FVOXXX(W3  ,W31 ,GWF,FMASS(2  ),FWIDTH(2  ),W41 )               
      CALL JIOXXX(W4  ,W41 ,GZN,ZMASS,ZWIDTH,W42 )                         
      CALL FVIXXX(W6  ,W42 ,GZD,FMASS(8  ),FWIDTH(8  ),W43 )               
      CALL IOVXXX(W43 ,W5  ,W7  ,GG,AMP(31 ))                              
      CALL FVOXXX(W5  ,W42 ,GZD,FMASS(8  ),FWIDTH(8  ),W44 )               
      CALL IOVXXX(W6  ,W44 ,W7  ,GG,AMP(32 ))                              
      CALL VVVXXX(W8  ,W27 ,W9  ,GWWA,AMP(33 ))                            
      CALL VVVXXX(W8  ,W27 ,W12 ,GWWZ,AMP(34 ))                            
      CALL VVVXXX(W8  ,W29 ,W9  ,GWWA,AMP(35 ))                            
      CALL VVVXXX(W8  ,W29 ,W12 ,GWWZ,AMP(36 ))                            
      CALL JVVXXX(W8  ,W31 ,GWWA,AMASS,AWIDTH,W45 )                        
      CALL FVIXXX(W6  ,W45 ,GAD,FMASS(8  ),FWIDTH(8  ),W46 )               
      CALL IOVXXX(W46 ,W5  ,W7  ,GG,AMP(37 ))                              
      CALL JVVXXX(W8  ,W31 ,GWWZ,ZMASS,ZWIDTH,W47 )                        
      CALL FVIXXX(W6  ,W47 ,GZD,FMASS(8  ),FWIDTH(8  ),W48 )               
      CALL IOVXXX(W48 ,W5  ,W7  ,GG,AMP(38 ))                              
      CALL FVOXXX(W5  ,W45 ,GAD,FMASS(8  ),FWIDTH(8  ),W49 )               
      CALL IOVXXX(W6  ,W49 ,W7  ,GG,AMP(39 ))                              
      CALL FVOXXX(W5  ,W47 ,GZD,FMASS(8  ),FWIDTH(8  ),W50 )               
      CALL IOVXXX(W6  ,W50 ,W7  ,GG,AMP(40 ))                              
      CALL FVIXXX(W6  ,W8  ,GWF,FMASS(7  ),FWIDTH(7  ),W51 )               
      CALL IOVXXX(W51 ,W5  ,W27 ,GWF,AMP(41 ))                             
      CALL IOVXXX(W51 ,W5  ,W29 ,GWF,AMP(42 ))                             
      CALL IOVXXX(W51 ,W15 ,W31 ,GWF,AMP(43 ))                             
      CALL FVOXXX(W5  ,W31 ,GWF,FMASS(7  ),FWIDTH(7  ),W52 )               
      CALL FVOXXX(W52 ,W8  ,GWF,FMASS(8  ),FWIDTH(8  ),W53 )               
      CALL IOVXXX(W6  ,W53 ,W7  ,GG,AMP(44 ))                              
      CALL IOVXXX(W51 ,W52 ,W7  ,GG,AMP(45 ))                              
      DUB_EMVESSBG = 0.D0 
      DO I = 1, NEIGEN
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NGRAPHS-5
              if (((j .gt.4) .and. (j .lt. 9))
     .              .or. ((j .gt.14) .and. (j .lt. 19))
     .              .or. ((j .gt.24) .and. (j .lt. 29))
     .              .or. ((j .gt.30) .and. (j .lt. 33))
     .              .or. ((j .gt.36) .and. (j .lt. 41))
     .              .or. (j .gt.42)) then
              continue
              else
              if ((zerowidth) .and. (((j .gt.20) .and. (j .lt. 33))
     .              .or. (j .gt. 40))) then
              continue
              else
              ZTEMP = ZTEMP + EIGEN_VEC(J,I)*AMP(J)
              endif
              endif
          ENDDO
          DUB_EMVESSBG =DUB_EMVESSBG+ZTEMP*EIGEN_VAL(I)*CONJG(ZTEMP) 
      ENDDO
C      CALL GAUGECHECK(AMP,ZTEMP,EIGEN_VEC,EIGEN_VAL,NGRAPHS,NEIGEN)
      END
