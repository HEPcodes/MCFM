      subroutine BDKfillmm(k1,k2,k3,k4,k5,k6,za,zb,ASLmm,Aslpp)
************************************************************************
*     Author: R.K. Ellis                                               *
*     August, 2010.                                                    *
************************************************************************
     ...
     & BDKfinite
      double precision msq,p2Dp3,s123,s234
     ...
     &   -zb(k5,k4)/s123*za(k1,k2)*zba2(k2,k1,k3,k6))
      ...
      lomm=mb/zb(k2,k3)*loreduced
      lopp=mb/za(k2,k3)*loreduced
...
C   *(-2*p2.p3*C0(p2,p3,m,0,m)-B0(p23,m,m)+B0(p3,0,m)+1/2*A0(m)/m^2);
     ...
     & +(-2d0*p2Dp3*C0(-1)+dcmplx(half))
      VV(0)=VV(0)
     & +(-2d0*p2Dp3*C0(0)-B0p23(0)+B0p3(0)+0.5d0*A(0)/msq)

      BDKfinite=BDK1211mm(k1,k2,k3,k4,k5,k6,za,zb)

      ASLmm(-2)=VV(-2)*lomm
      ASLmm(-1)=VV(-1)*lomm
      ASLmm(0)=VV(0)*lomm+BDKfinite*mb/zb(k2,k3)

      ASLpp(-2)=VV(-2)*lopp
      ASLpp(-1)=VV(-1)*lopp
      ASLpp(0)=VV(0)*lopp+BDKfinite*mb/za(k2,k3)
      return
      end
