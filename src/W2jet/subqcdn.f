...
c      aRLAB=[TATB]*(2d0*sw^2*gzle*Lu)*qcdab(2,1,1)
     ...
     & -za(p2,p6)*zb(p1,p4)*zb(p5,p6)*aba23)/t256
      qcdab(2,1,1)=2d0*qcdab(2,1,1)/za(p2,p5)/s34/t56

c      aLLBA=[TBTA]*qcdba(1,1,1)*(2d0*sw^2*gzle*Lu)
      qcdba(1,1,1)=
     & (-2d0*za(p2,p5)*za(p3,p4)*zb(p1,p4)*zb(p1,p4)*nDp5
     ...
     & *za(p2,p3)*za(p5,p6)*zb(p1,p6))/t156
      qcdba(1,1,1)=2d0*qcdba(1,1,1)/zb(p1,p5)/t56/s34

c      aRLBA=[TBTA]*(2d0*sw^2*gzle*Lu)*qcdba(2,1,1)
      qcdba(2,1,1)=
     ...
     & +(-zba(p5,p2)*za(p2,p3)*zb(p1,p6)*ab64
     & +2d0*nDp5*za(p2,p3)*za(p2,p3)*zb(p1,p5)*zb(p3,p4))
     & /za(p2,p5))/t156
     ...
     & *ab36/za(p2,p5))/t256
      qcdba(2,1,1)=2d0*qcdba(2,1,1)/t56/s34

      if ((nwz .eq. +1) .or. (nwz .eq. -1)) return
...
c      aRRAB = qcdab(2,2,1)*sw/cw*(-2d0*Qu*sw^2)*gzle
     ...
     & *(2d0/t256/t56*nDp5)
      qcdab(2,2,1) = 2d0*qcdab(2,2,1)/s34
...
c      aRRBA = qcdba(2,2,1)*sw/cw*(-2d0*Qu*sw^2)*gzle
     ...
     &  *2d0*nDp5/t256/t56 
      ...
      qcdab(2,2,2)=Dconjg(qcdab(1,1,1))
      qcdab(1,1,2)=Dconjg(qcdab(2,2,1))
      qcdab(1,2,2)=Dconjg(qcdab(2,1,1))
      qcdab(2,1,2)=Dconjg(qcdab(1,2,1))

      qcdba(2,2,2)=Dconjg(qcdba(1,1,1))
      qcdba(1,1,2)=Dconjg(qcdba(2,2,1))
      qcdba(1,2,2)=Dconjg(qcdba(2,1,1))
      qcdba(2,1,2)=Dconjg(qcdba(1,2,1))
