...
c--- all momenta are incoming
     ...
     .                 gqWqg2_cs(0:2),ggWqbq2_cs(0:2)
      double precision qqb_ijkk,qqb_ijii,qqb_ijjj,qqb_ijkj,qqb_ijik,
     .                 qqb_ijkl,qqb_iiij,qqb_iiji
      double precision qbq_ijkk,qbq_ijii,qbq_ijjj,qbq_ijkj,qbq_ijik,
     .                 qbq_ijkl,qbq_iiij,qbq_iiji
      double precision qq_iiji,qq_ijkj,qq_ijik,qq_ijjj,qq_ijii
      double precision qbqb_iiji,qbqb_ijkj,qbqb_ijik,qbqb_ijjj,qbqb_ijii
...
c--- now square these amplitudes
c
c--- q(i) qb(j) --> W + g* (--> q(k) qb(k)) with k != i,j
        qqb_ijkk=abs(qqb1(1))**2+abs(qqb3(1))**2 
        qbq_ijkk=abs(qbq1(1))**2+abs(qbq3(1))**2 
c--- q(i) qb(j) --> W + g* (--> q(i) qb(i)) i.e. k = i
        qqb_ijii=abs(qqb1(1))**2+abs(qqb3(1))**2
     .          +abs(qqb2(2))**2+abs(qqb4(2))**2
     .          +2d0/xn*(qqb1(1)*Dconjg(qqb2(2)))
        qbq_ijii=abs(qbq1(1))**2+abs(qbq3(1))**2
     .          +abs(qbq2(2))**2+abs(qbq4(2))**2
     .          +2d0/xn*(qbq1(1)*Dconjg(qbq2(2)))
c--- q(i) qb(j) --> W + g* (--> q(j) qb(j)) i.e. k = j
        qqb_ijjj=abs(qqb1(1))**2+abs(qqb3(1))**2
     .          +abs(qqb2(3))**2+abs(qqb4(3))**2
     .          +2d0/xn*(qqb1(1)*Dconjg(qqb2(3)))
        qbq_ijjj=abs(qbq1(1))**2+abs(qbq3(1))**2
     .          +abs(qbq2(3))**2+abs(qbq4(3))**2
     .          +2d0/xn*(qbq1(1)*Dconjg(qbq2(3)))
c--- q (i) qb(j) --> q(i) qb(j) ( --> W qb(k)) with k != i,j
        qqb_ijik=abs(qqb2(2))**2+abs(qqb4(2))**2 
        qbq_ijik=abs(qbq2(2))**2+abs(qbq4(2))**2 
c--- q (i) qb(j) --> q(i) ( --> W q(k)) qb(j) with k != i,j
        qqb_ijkj=abs(qqb2(3))**2+abs(qqb4(3))**2 
        qbq_ijkj=abs(qbq2(3))**2+abs(qbq4(3))**2 
        
c--- q(i) qb(j) --> g* --> q(l) (--> W q(k)) qb(l)
        qqb_ijkl=abs(qqb2(1))**2+abs(qqb4(1))**2
        qbq_ijkl=abs(qbq2(1))**2+abs(qbq4(1))**2
c--- q(i) qb(i) --> g* --> q(j) (--> W q(i)) qb(j)
        qqb_iiij=abs(qqb2(1))**2+abs(qqb4(1))**2            
     .          +abs(qqb1(3))**2+abs(qqb3(3))**2 
     .          +2d0/xn*(qqb2(1)*Dconjg(qqb1(3)))          
        qbq_iiij=abs(qbq2(1))**2+abs(qbq4(1))**2            
     .          +abs(qbq1(3))**2+abs(qbq3(3))**2 
     .          +2d0/xn*(qbq2(1)*Dconjg(qbq1(3)))  
c--- q(i) qb(i) --> g* --> q(j) qb(j) (--> W qb(i))
        qqb_iiji=abs(qqb2(1))**2+abs(qqb4(1))**2            
     .          +abs(qqb1(2))**2+abs(qqb3(2))**2 
     .          +2d0/xn*(qqb2(1)*Dconjg(qqb1(2))) 
        qbq_iiji=abs(qbq2(1))**2+abs(qbq4(1))**2            
     .          +abs(qbq1(2))**2+abs(qbq3(2))**2 
     .          +2d0/xn*(qbq2(1)*Dconjg(qbq1(2)))  
     
c--- q(i) q(i) --> q(i) ( --> W q(j) ) q(i)
        qq_iiji=abs(qq1(1))**2+abs(qq3(1))**2
     .         +abs(qq1(2))**2+abs(qq3(2))**2
     .         +2d0/xn*(qq1(1)*Dconjg(qq1(2)))
c--- q(i) q(j) --> q(i) ( --> W q(k) ) q(j)
        qq_ijkj=abs(qq1(1))**2+abs(qq3(1))**2
c--- q(i) q(j) --> q(i) q(j) ( --> W q(k) )
        qq_ijik=abs(qq1(3))**2+abs(qq3(3))**2
c--- q(i) q(j) --> q(i) ( --> W q(j) ) q(j)
        qq_ijjj=abs(qq1(1))**2+abs(qq3(1))**2
     .         +abs(qq1(4))**2+abs(qq3(4))**2
     .          +2d0/xn*(qq1(1)*Dconjg(qq1(4)))  
c--- q(i) q(j) --> q(i) q(j) ( --> W q(i) ) 
        qq_ijii=abs(qq1(2))**2+abs(qq3(2))**2
     .         +abs(qq1(3))**2+abs(qq3(3))**2
     .          +2d0/xn*(qq1(2)*Dconjg(qq1(3)))  

c--- qb(i) qb(i) --> qb(i) ( --> W qb(j) ) qb(i)
        qbqb_iiji=abs(qbqb1(1))**2+abs(qbqb3(1))**2
     .           +abs(qbqb1(2))**2+abs(qbqb3(2))**2
     .           +2d0/xn*(qbqb1(1)*Dconjg(qbqb1(2)))
c--- qb(i) qb(j) --> qb(i) ( --> W qb(k) ) qb(j)
        qbqb_ijkj=abs(qbqb1(1))**2+abs(qbqb3(1))**2
c--- qb(i) qb(j) --> qb(i) qb(j) ( --> W qb(k) )
        qbqb_ijik=abs(qbqb1(3))**2+abs(qbqb3(3))**2
c--- qb(i) qb(j) --> qb(i) ( --> W qb(j) ) qb(j)
        qbqb_ijjj=abs(qbqb1(1))**2+abs(qbqb3(1))**2
     .         +abs(qbqb1(4))**2+abs(qbqb3(4))**2
     .          +2d0/xn*(qbqb1(1)*Dconjg(qbqb1(4)))  
c--- qb(i) qb(j) --> qb(i) qb(j) ( --> W qb(i) ) 
        qbqb_ijii=abs(qbqb1(2))**2+abs(qbqb3(2))**2
     .         +abs(qbqb1(3))**2+abs(qbqb3(3))**2
     .          +2d0/xn*(qbqb1(2)*Dconjg(qbqb1(3)))  
      endif
...
c--- Q QBAR - different flavours
          msq(j,k)=facqq*Vsq(j,k)*half*(
     .             (nf-2)*qqb_ijkk+qqb_ijii+qqb_ijjj)
     .            +facqq*Vsum(j)*qqb_ijkj
     .            +facqq*Vsum(k)*qqb_ijik
c          msq(j,k)=facqq*(qqb_ijkk+qqb_ijii+qqb_ijjj)
c     .            +facqq*qqb_ijik
c     .            +facqq*qqb_ijkj
        else
c--- Q QBAR - same flavours
          ...
          enddo
          enddo
          msq(j,k)=facqq*Vsum(k)*qqb_iiij
     .            +facqq*Vsum(j)*qqb_iiji
     .            +facqq*Vfac*qqb_ijkl
c     .            +facqq*qqb_iiij
c     .            +facqq*qqb_iiji
c     .            +facqq*qqb_ijkl
...
c--- QBAR Q - different flavours
          msq(j,k)=facqq*Vsq(j,k)*half*(
     .             (nf-2)*qbq_ijkk+qbq_ijii+qbq_ijjj)
     .            +facqq*Vsum(j)*qbq_ijkj
     .            +facqq*Vsum(k)*qbq_ijik
c          msq(j,k)=facqq*(qbq_ijkk+qbq_ijii+qbq_ijjj)
c     .            +facqq*qbq_ijik
c     .            +facqq*qbq_ijkj
        else
c--- QBAR Q - same flavours
          ...
          enddo
          enddo
          msq(j,k)=facqq*Vsum(k)*qbq_iiij
     .            +facqq*Vsum(j)*qbq_iiji
     .            +facqq*Vfac*qbq_ijkl
c     .            +facqq*qbq_ijkl
c     .            +facqq*qbq_iiji
c     .            +facqq*qbq_iiij
...
c--- Q Q - different flavours
          msq(j,k)=facqq*(Vsum(j)-Vsq(j,-k))*qq_ijkj
     .            +facqq*(Vsum(k)-Vsq(k,-j))*qq_ijik
     .            +facqq*half*Vsq(j,-k)*qq_ijjj
     .            +facqq*half*Vsq(k,-j)*qq_ijii
c     .            +facqq*qq_ijkj
c     .            +facqq*qq_ijik
c     .            +facqq*half*qq_ijjj
c     .            +facqq*half*qq_ijii
        else
c--- Q Q - same flavours
          msq(j,k)=facqq*Vsum(j)*qq_iiji
c     .            +facqq*qq_iiji
        endif
      elseif ((j .lt. 0) .and. (k .lt. 0)) then
        if (j. ne. k) then
c--- QBAR QBAR - different flavours
          msq(j,k)=facqq*(Vsum(j)-Vsq(j,-k))*qbqb_ijkj
     .            +facqq*(Vsum(k)-Vsq(k,-j))*qbqb_ijik
     .            +facqq*half*Vsq(j,-k)*qbqb_ijjj
     .            +facqq*half*Vsq(k,-j)*qbqb_ijii
c     .            +facqq*qbqb_ijkj
c     .            +facqq*qbqb_ijik
c     .            +facqq*half*qbqb_ijjj
c     .            +facqq*half*qbqb_ijii
        else
c--- QBAR QBAR - same flavours
          msq(j,k)=facqq*Vsum(j)*qbqb_iiji
c     .            +facqq*qbqb_iiji
        endif
      endif
      enddo
