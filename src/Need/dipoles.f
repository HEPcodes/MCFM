...
c--- new version (2/22/00)
        ii_qg=epinv*(epinv-L)+0.5d0*L**2+1.5d0*epinv-half-pisqo6
        return
      endif
...
c--- divergent for _v (vorz=1) or finite for _z (vorz=2,3 for reg,plus)     
      ...
      if (vorz .eq. 1) then
        ii_gg=epinv*(epinv-L)+half*L**2-pisqo6
...
c--- new version (2/22/00)
        if_qg=epinv*(epinv-L)+half*L**2+1.5d0*epinv-half+pisqo6
        return
      endif
...
c--- divergent for _v (vorz=1) or finite for _z (vorz=2,3 for reg,plus)     
      ...
      if (vorz .eq. 1) then
        if_gg=epinv*(epinv-L)+half*L**2+pisq/6d0
     .       +(11d0-two*nf/xn)/6d0*epinv
        return
      endif
...
c--- divergent for _v (vorz=1) or finite for _z (vorz=2,3 for reg,plus)     
      ...
      if (vorz .eq. 1) then
        fi_qg=epinv*(epinv-L)+half*L**2+1.5d0*(epinv-L)-half*pisq+3d0
         return
      endif
...
c--- divergent for _v (vorz=1) or finite for _z (vorz=2,3 for reg,plus)     
      ...
      if (vorz .eq. 1) then
        fi_gg=two*(epinv*(epinv-L)+half*L**2)-pisq+67d0/9d0
        return
      endif
...
c--- divergent for _v (vorz=1) or finite for _z (vorz=2,3 for reg,plus)     
      ...
      ff_qg=0d0
      if (vorz .eq. 1) then
        ff_qg=epinv*(epinv-L)+half*L**2+1.5d0*(epinv-L)+4.5d0-half*pisq
      endif
      return
      end
...
c--- divergent for _v (vorz=1) or finite for _z (vorz=2,3 for reg,plus)     
      ...
      ff_gq=0d0
      if (vorz .eq. 1) then
        ff_gq=2d0/3d0*(epinv-L)+19d0/9d0
      endif
      return
      end
...
c--- divergent for _v (vorz=1) or finite for _z (vorz=2,3 for reg,plus)     
      ...
      ff_gg=0d0
      if (vorz .eq. 1) then
        ff_gg=two*(epinv*(epinv-L)+half*L**2)+11d0/3d0*(epinv-L)
     & +100d0/9d0-pisq
      endif
      return
      end
