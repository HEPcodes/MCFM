      subroutine wamp(p1,p2,ie,in,jn,je,jb,amp)
c     g(-p1)+s(-p2) --> e^-(ie)+n(in)+Pn(jn)+Pe^+(je)+b(jb)
      ...
      integer j
      double precision propp,propd,propt,taugt,tsq
...
c---  amp(1)= negative helicity, amp(2)= positive helicity    
      amp(1) =
     &  + 1/(zb(p1,p2))*za(p1,ie)*za(jn,jb)*zb(p2,in)*zb(p2,je)*mt**2*
     & taugt**(-1)
     ...
     &
      ...
      amp(2) =
     &  - 1/(za(p1,p2))*za(p2,ie)*za(jn,jb)*zb(p1,je)*zb(p2,in)*mt**2*
     & taugt**(-1)
