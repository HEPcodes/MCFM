      double complex function A1ggggmmmmCC(j1,j2,j3,j4,za,zb)
      implicit none
...
c---                          F42me(s,t;Psq,Qsq) -> F42me(Psq,Qsq;s,t)
     ...
     .                               sc(ii(i+1),ii(i+2)))
      ...
      double complex function A1ggggmmmmNCC(j1,j2,j3,j4,za,zb)
      implicit none
...
C--- Expresssion of Eq. (25) of Badger and Glover, hep-ph/0607139v2
     ...
     . +(za(j3,j4)/zb(j1,j2))**2
     . +2d0*za(j3,j4)*za(j4,j1)/(zb(j1,j2)*zb(j2,j3))
     . +(s(j1,j2)*s(j3,j4)+s3(j1,j2,j3)*s3(j2,j3,j4)-s(j1,j2)**2)
     . /(2d0*zb(j1,j2)*zb(j2,j3)*zb(j3,j4)*zb(j4,j1))
