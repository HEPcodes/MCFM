...
c***************************************************************************
c     Fake version of cernlibs pdfset. In this sets common block value ISET
c***************************************************************************
      implicit none
c
c     Arguments
...
c  Begin Code
c-----
      call Ctq5Pds(Iset, Pdf, X, Q, Irt)
c
c     Now switch so that d is 1 and u is 2
c
      temp=pdf(1)
      pdf(1)=pdf(2)
      pdf(2)=temp
...
c
      logical first_time
      double precision alpha_fixed

      data first_time/.true./
      data alpha_fixed /.127d0/
c-----
c  Begin Code
c-----
      if (first_time) then
         first_time=.false.
         write(*,*) 'Using fixed alpha_s = ',alpha_fixed
      endif
      alphas2 = alpha_fixed
      end
