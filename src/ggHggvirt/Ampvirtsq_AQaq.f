      double precision function Ampvirtsq_AQaq_nonid(p1,p2,p3,p4)
...
c--- equivalent to complex conjugation for Born amplitudes    
      amplo(2,1)=dconjg(amplo(1,2))
      amplo(2,2)=dconjg(amplo(1,1))
...
c---   overall 4*C^2*gs^4*ason2pi is applied in gg_hgg_v.f)
      ...
      sum=sum+dble(ampv(h1,h2)*dconjg(amplo(h1,h2)))
      enddo
      enddo
      
      Ampvirtsq_AQaq_nonid=sum*V*xn/4d0
                  
      return
      end


      double precision function Ampvirtsq_AQaq_ident(p1,p2,p3,p4)
c--- this routine is a wrapper to the 1-loop/Born interference
c--- of D.&S. 0906.0008; it performs the interference and sum over
c--- helicities and returns the ME in the same fashion as Hqarbvsq
      implicit none
...
c--- equivalent to complex conjugation for Born amplitudes    
      amplo_a(2,2)=dconjg(amplo_a(1,1))
...
c--- equivalent to complex conjugation for Born amplitudes    
      amplo_b(2,2)=dconjg(amplo_b(1,1))
...
c      sum=sum+dble(amp41_a(h1,h2)*dconjg(amplo_a(h1,h2)))
c     .       +dble(amp41_b(h1,h2)*dconjg(amplo_b(h1,h2)))
c      if (h1 .eq. h2) then
      sum=sum-1d0/xn*dble(amp42_a(h1,h2)*dconjg(amplo_b(h1,h2)))
     .       -1d0/xn*dble(amp42_b(h1,h2)*dconjg(amplo_a(h1,h2)))      
c      endif
c      enddo
      enddo
      
      Ampvirtsq_AQaq_ident=sum*V*xn/4d0
