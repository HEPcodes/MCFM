...
c---
     ...
     & qqb(2),qbq(2),gq(2),gqb(2),qg(2),qbg(2)
      double complex pr345,prp34,cprop,
...
c--   calculate propagator factors
      ...
      pr345=s345/Dcmplx((s345-zmass**2),zmass*zwidth)
c--   correction factor due to final-state Z not being exactly on-shell
      cprop=(s345-s34)/Dcmplx((s345-zmass**2),zmass*zwidth)
...
c--- note that af=0 when zerowidth=.true.
      ...
      double precision s126,s34,s345,s156,s256,xfac
      double complex ai(2,2,2,2),af(2,2,2,2),
...
c--- basic
      anomZZ(1,1,2,2)=1d0/(4d0*s34*za(p1,p6)*za(p2,p6))
     & *((im*h1tZ+h3tZ)*2d0*za(p1,p3)*zb(p4,p5)*zazb(p1,p2,p6,p5)
     &  +(im*h2tZ+h4tZ)*za(p3,p5)*zb(p5,p4)*zazb(p1,p2,p6,p5)**2)
c--- anomalous couplings, hep-ph/0002138 Eq.(22) * (-i)
      anomZZ(1,1,2,1)=1d0/(4d0*s34*zb(p1,p6)*zb(p2,p6))
     & *((im*h1tZ+h3tZ)*2d0*zb(p4,p5)*zb(p2,p5)*zazb(p3,p1,p6,p2)
...
c      anomZZ(1,2,2,2)=1d0/(4d0*s34*za(p1,p6)*za(p2,p6))
c     & *((im*h1tZ+h3tZ)*2d0*za(p1,p4)*zb(p3,p5)*zazb(p1,p2,p6,p5)
c     &  +(im*h2tZ+h4tZ)*za(p4,p5)*zb(p5,p3)*zazb(p1,p2,p6,p5)**2)
      anomZZ(2,1,1,1)=1d0/(4d0*s34*zb(p1,p6)*zb(p2,p6))
     & *((-im*h1tZ+h3tZ)*2d0*zb(p1,p4)*za(p3,p5)*zbza(p1,p2,p6,p5)
     &  +(-im*h2tZ+h4tZ)*zb(p4,p5)*za(p5,p3)*zbza(p1,p2,p6,p5)**2)
      anomZZ(1,2,2,1)=1d0/(4d0*s34*zb(p1,p6)*zb(p2,p6))
     & *((im*h1tZ+h3tZ)*2d0*zb(p3,p5)*zb(p2,p5)*zazb(p4,p1,p6,p2)
     &  +(im*h2tZ+h4tZ)*s345*za(p4,p5)*zb(p5,p3)*zb(p2,p5)**2)
C Flip_2: (za<-->zb),(1<-->2),(3<-->4) and h1t -> -h1t, h2t -> -h2t
      anomZZ(1,1,1,1)=1d0/(4d0*s34*zb(p2,p6)*zb(p1,p6))
     & *((-im*h1tZ+h3tZ)*2d0*zb(p2,p4)*za(p3,p5)*zbza(p2,p1,p6,p5)
     &  +(-im*h2tZ+h4tZ)*zb(p4,p5)*za(p5,p3)*zbza(p2,p1,p6,p5)**2)
      anomZZ(1,1,1,2)=1d0/(4d0*s34*za(p2,p6)*za(p1,p6))
     & *((-im*h1tZ+h3tZ)*2d0*za(p3,p5)*za(p1,p5)*zbza(p4,p2,p6,p1)
     &  +(-im*h2tZ+h4tZ)*s345*zb(p4,p5)*za(p5,p3)*za(p1,p5)**2)
C (za<-->zb),(1<-->2) and h1t -> -h1t, h2t -> -h2t
      anomZZ(1,2,1,1)=1d0/(4d0*s34*zb(p2,p6)*zb(p1,p6))
     & *((-im*h1tZ+h3tZ)*2d0*zb(p2,p3)*za(p4,p5)*zbza(p2,p1,p6,p5)
     &  +(-im*h2tZ+h4tZ)*zb(p3,p5)*za(p5,p4)*zbza(p2,p1,p6,p5)**2)
      anomZZ(1,2,1,2)=1d0/(4d0*s34*za(p2,p6)*za(p1,p6))
     & *((-im*h1tZ+h3tZ)*2d0*za(p4,p5)*za(p1,p5)*zbza(p3,p2,p6,p1)
     &  +(-im*h2tZ+h4tZ)*s345*zb(p3,p5)*za(p5,p4)*za(p1,p5)**2)
      
      anomZZ(1,2,2,2)=Dconjg(anomZZ(2,1,1,1))
      anomZZ(2,1,2,2)=Dconjg(anomZZ(1,2,1,1))
      anomZZ(2,2,1,2)=Dconjg(anomZZ(1,1,2,1))
      anomZZ(2,2,2,1)=Dconjg(anomZZ(1,1,1,2))

      anomZZ(2,2,2,2)=Dconjg(anomZZ(1,1,1,1))
      anomZZ(2,2,1,1)=Dconjg(anomZZ(1,1,2,2))
      anomZZ(2,1,2,1)=Dconjg(anomZZ(1,2,1,2))
      anomZZ(2,1,1,2)=Dconjg(anomZZ(1,2,2,1))

c--- diagrams qq~ -> gam* -> Z+gam
c--- basic
      anomgamZ(1,1,2,2)=1d0/(4d0*s34*za(p1,p6)*za(p2,p6))
     & *((im*h1tgam+h3tgam)*2d0*za(p1,p3)*zb(p4,p5)*zazb(p1,p2,p6,p5)
     &  +(im*h2tgam+h4tgam)*za(p3,p5)*zb(p5,p4)*zazb(p1,p2,p6,p5)**2)
c--- anomalous couplings, hep-ph/0002138 Eq.(22) * (-i)
      anomgamZ(1,1,2,1)=1d0/(4d0*s34*zb(p1,p6)*zb(p2,p6))
     & *((im*h1tgam+h3tgam)*2d0*zb(p4,p5)*zb(p2,p5)*zazb(p3,p1,p6,p2)
     &  +(im*h2tgam+h4tgam)*s345*za(p3,p5)*zb(p5,p4)*zb(p2,p5)**2)
C (3<-->4) ! but manually perform symmetry of (1,2,2,2) to (2,1,1,1)
           ! in order to use the same relations as for the SM amplitudes
c      anomgamZ(1,2,2,2)=1d0/(4d0*s34*za(p1,p6)*za(p2,p6))
c     & *((im*h1tgam+h3tgam)*2d0*za(p1,p4)*zb(p3,p5)*zazb(p1,p2,p6,p5)
c     &  +(im*h2tgam+h4tgam)*za(p4,p5)*zb(p5,p3)*zazb(p1,p2,p6,p5)**2)
      anomgamZ(2,1,1,1)=1d0/(4d0*s34*zb(p1,p6)*zb(p2,p6))
     & *((-im*h1tgam+h3tgam)*2d0*zb(p1,p4)*za(p3,p5)*zbza(p1,p2,p6,p5)
     &  +(-im*h2tgam+h4tgam)*zb(p4,p5)*za(p5,p3)*zbza(p1,p2,p6,p5)**2)
      anomgamZ(1,2,2,1)=1d0/(4d0*s34*zb(p1,p6)*zb(p2,p6))
     & *((im*h1tgam+h3tgam)*2d0*zb(p3,p5)*zb(p2,p5)*zazb(p4,p1,p6,p2)
     &  +(im*h2tgam+h4tgam)*s345*za(p4,p5)*zb(p5,p3)*zb(p2,p5)**2)
C Flip_2: (za<-->zb),(1<-->2),(3<-->4) and h1t -> -h1t, h2t -> -h2t
      anomgamZ(1,1,1,1)=1d0/(4d0*s34*zb(p2,p6)*zb(p1,p6))
     & *((-im*h1tgam+h3tgam)*2d0*zb(p2,p4)*za(p3,p5)*zbza(p2,p1,p6,p5)
     &  +(-im*h2tgam+h4tgam)*zb(p4,p5)*za(p5,p3)*zbza(p2,p1,p6,p5)**2)
      anomgamZ(1,1,1,2)=1d0/(4d0*s34*za(p2,p6)*za(p1,p6))
     & *((-im*h1tgam+h3tgam)*2d0*za(p3,p5)*za(p1,p5)*zbza(p4,p2,p6,p1)
     &  +(-im*h2tgam+h4tgam)*s345*zb(p4,p5)*za(p5,p3)*za(p1,p5)**2)
C (za<-->zb),(1<-->2) and h1t -> -h1t, h2t -> -h2t
      anomgamZ(1,2,1,1)=1d0/(4d0*s34*zb(p2,p6)*zb(p1,p6))
     & *((-im*h1tgam+h3tgam)*2d0*zb(p2,p3)*za(p4,p5)*zbza(p2,p1,p6,p5)
     &  +(-im*h2tgam+h4tgam)*zb(p3,p5)*za(p5,p4)*zbza(p2,p1,p6,p5)**2)
      anomgamZ(1,2,1,2)=1d0/(4d0*s34*za(p2,p6)*za(p1,p6))
     & *((-im*h1tgam+h3tgam)*2d0*za(p4,p5)*za(p1,p5)*zbza(p3,p2,p6,p1)
     &  +(-im*h2tgam+h4tgam)*s345*zb(p3,p5)*za(p5,p4)*za(p1,p5)**2)
      
      anomgamZ(1,2,2,2)=Dconjg(anomgamZ(2,1,1,1))
      anomgamZ(2,1,2,2)=Dconjg(anomgamZ(1,2,1,1))
      anomgamZ(2,2,1,2)=Dconjg(anomgamZ(1,1,2,1))
      anomgamZ(2,2,2,1)=Dconjg(anomgamZ(1,1,1,2))

      anomgamZ(2,2,2,2)=Dconjg(anomgamZ(1,1,1,1))
      anomgamZ(2,2,1,1)=Dconjg(anomgamZ(1,1,2,2))
      anomgamZ(2,1,2,1)=Dconjg(anomgamZ(1,2,1,2))
      anomgamZ(2,1,1,2)=Dconjg(anomgamZ(1,2,2,1))

c--- diagrams qq~ -> Z -> gam*+gam
C basic
      anomZgam(1,1,2,2)=-1d0/(4d0*s126*za(p1,p6)*za(p2,p6))
     & *zb(p4,p5)*(za(p1,p2)*zb(p2,p5)+za(p1,p6)*zb(p6,p5))
     & *((im*h1tgam+h3tgam)*2d0*za(p1,p3)
     &  +(im*h2tgam+h4tgam)*za(p3,p4)*zb(p4,p5)*za(p1,p5))
      anomZgam(1,1,2,1)=-1d0/(4d0*s126*zb(p1,p6)*zb(p2,p6))
     & *zb(p4,p5)*zb(p2,p5)
     & *((im*h1tgam+h3tgam)
     &   *2d0*(za(p3,p1)*zb(p1,p2)+za(p3,p6)*zb(p6,p2))
     &  +(im*h2tgam+h4tgam)
     &   *za(p3,p4)*zb(p4,p5)*(za(p5,p1)*zb(p1,p2)+za(p5,p6)*zb(p6,p2)))
C (3<-->4) ! but manually perform symmetry of (1,2,2,2) to (2,1,1,1)
           ! in order to use the same relations as for the SM amplitudes
      anomZgam(2,1,1,1)=-1d0/(4d0*s126*zb(p1,p6)*zb(p2,p6))
     & *za(p3,p5)*(zb(p1,p2)*za(p2,p5)+zb(p1,p6)*za(p6,p5))
     & *((-im*h1tgam+h3tgam)*2d0*zb(p1,p4)
     &  +(-im*h2tgam+h4tgam)*zb(p4,p3)*za(p3,p5)*zb(p1,p5))
      anomZgam(1,2,2,1)=-1d0/(4d0*s126*zb(p1,p6)*zb(p2,p6))
     & *zb(p3,p5)*zb(p2,p5)
     & *((im*h1tgam+h3tgam)
     &   *2d0*(za(p4,p1)*zb(p1,p2)+za(p4,p6)*zb(p6,p2))
     &  +(im*h2tgam+h4tgam)
     &   *za(p4,p3)*zb(p3,p5)*(za(p5,p1)*zb(p1,p2)+za(p5,p6)*zb(p6,p2)))
C Flip_2: (za<-->zb),(1<-->2),(3<-->4) and h1t -> -h1t, h2t -> -h2t
      anomZgam(1,1,1,1)=-1d0/(4d0*s126*zb(p1,p6)*zb(p2,p6))
     & *za(p3,p5)*(zb(p2,p1)*za(p1,p5)+zb(p2,p6)*za(p6,p5))
     & *((-im*h1tgam+h3tgam)*2d0*zb(p2,p4)
     &  +(-im*h2tgam+h4tgam)*zb(p4,p3)*za(p3,p5)*zb(p2,p5))
      anomZgam(1,1,1,2)=-1d0/(4d0*s126*za(p1,p6)*za(p2,p6))
     & *za(p3,p5)*za(p1,p5)
     & *((-im*h1tgam+h3tgam)
     &   *2d0*(zb(p4,p2)*za(p2,p1)+zb(p4,p6)*za(p6,p1))
     &  +(-im*h2tgam+h4tgam)
     &   *zb(p4,p3)*za(p3,p5)*(zb(p5,p2)*za(p2,p1)+zb(p5,p6)*za(p6,p1)))
C (za<-->zb),(1<-->2) and h1t -> -h1t, h2t -> -h2t
      anomZgam(1,2,1,1)=-1d0/(4d0*s126*zb(p1,p6)*zb(p2,p6))
     & *za(p4,p5)*(zb(p2,p1)*za(p1,p5)+zb(p2,p6)*za(p6,p5))
     & *((-im*h1tgam+h3tgam)*2d0*zb(p2,p3)
     &  +(-im*h2tgam+h4tgam)*zb(p3,p4)*za(p4,p5)*zb(p2,p5))
      anomZgam(1,2,1,2)=-1d0/(4d0*s126*za(p1,p6)*za(p2,p6))
     & *za(p4,p5)*za(p1,p5)
     & *((-im*h1tgam+h3tgam)
     &   *2d0*(zb(p3,p2)*za(p2,p1)+zb(p3,p6)*za(p6,p1))
     &  +(-im*h2tgam+h4tgam)
     &   *zb(p3,p4)*za(p4,p5)*(zb(p5,p2)*za(p2,p1)+zb(p5,p6)*za(p6,p1)))
     
      anomZgam(1,2,2,2)=Dconjg(anomZgam(2,1,1,1))
      anomZgam(2,1,2,2)=Dconjg(anomZgam(1,2,1,1))
      anomZgam(2,2,1,2)=Dconjg(anomZgam(1,1,2,1))
      anomZgam(2,2,2,1)=Dconjg(anomZgam(1,1,1,2))

      anomZgam(2,2,2,2)=Dconjg(anomZgam(1,1,1,1))
      anomZgam(2,2,1,1)=Dconjg(anomZgam(1,1,2,2))
      anomZgam(2,1,2,1)=Dconjg(anomZgam(1,2,1,2))
      anomZgam(2,1,1,2)=Dconjg(anomZgam(1,2,2,1))
