...
c--- and the fourth power for h2,h4 (dim. 8)
      if (tevscale .gt. 0d0) then
        xfac=1d0/(1d0+s12/(tevscale*1d3)**2)
      else
        xfac=1d0
      endif
...
C basic
      anomZZ(1,1,2)=1d0/4d0/s34
     & *((im*h1tZ+h3tZ)*2d0*za(p1,p3)*zb(p2,p5)*zb(p4,p5)
     &  +(im*h2tZ+h4tZ)*za(p1,p2)*za(p3,p5)*zb(p5,p4)*zb(p2,p5)**2)
C (3<-->4)
      anomZZ(1,2,2)=1d0/4d0/s34
     & *((im*h1tZ+h3tZ)*2d0*za(p1,p4)*zb(p2,p5)*zb(p3,p5)
     &  +(im*h2tZ+h4tZ)*za(p1,p2)*za(p4,p5)*zb(p5,p3)*zb(p2,p5)**2)
C Flip_2: (za<-->zb),(1<-->2),(3<-->4) and h1t -> -h1t, h2t -> -h2t
      anomZZ(1,1,1)=1d0/4d0/s34
     & *((-im*h1tZ+h3tZ)*2d0*zb(p2,p4)*za(p1,p5)*za(p3,p5)
     &  +(-im*h2tZ+h4tZ)*zb(p2,p1)*zb(p4,p5)*za(p5,p3)*za(p1,p5)**2)
C (za<-->zb),(1<-->2)
      anomZZ(1,2,1)=1d0/4d0/s34
     & *((-im*h1tZ+h3tZ)*2d0*zb(p2,p3)*za(p1,p5)*za(p4,p5)
     &  +(-im*h2tZ+h4tZ)*zb(p2,p1)*zb(p3,p5)*za(p5,p4)*za(p1,p5)**2)

c--- diagrams qq~ -> gam* -> Z+gam
C basic
      anomgamZ(1,1,2)=1d0/4d0/s34
     & *((im*h1tgam+h3tgam)*2d0*za(p1,p3)*zb(p2,p5)*zb(p4,p5)
     &  +(im*h2tgam+h4tgam)*za(p1,p2)*za(p3,p5)*zb(p5,p4)*zb(p2,p5)**2)
C (3<-->4)
      anomgamZ(1,2,2)=1d0/4d0/s34
     & *((im*h1tgam+h3tgam)*2d0*za(p1,p4)*zb(p2,p5)*zb(p3,p5)
     &  +(im*h2tgam+h4tgam)*za(p1,p2)*za(p4,p5)*zb(p5,p3)*zb(p2,p5)**2)
C Flip_2: (za<-->zb),(1<-->2),(3<-->4) and h1t -> -h1t, h2t -> -h2t
      anomgamZ(1,1,1)=1d0/4d0/s34
     & *((-im*h1tgam+h3tgam)*2d0*zb(p2,p4)*za(p1,p5)*za(p3,p5)
     &  +(-im*h2tgam+h4tgam)*zb(p2,p1)*zb(p4,p5)*za(p5,p3)*za(p1,p5)**2)
C (za<-->zb),(1<-->2)
      anomgamZ(1,2,1)=1d0/4d0/s34
     & *((-im*h1tgam+h3tgam)*2d0*zb(p2,p3)*za(p1,p5)*za(p4,p5)
     &  +(-im*h2tgam+h4tgam)*zb(p2,p1)*zb(p3,p5)*za(p5,p4)*za(p1,p5)**2)

c--- diagrams qq~ -> Z -> gam*+gam
C basic
      anomZgam(1,1,2)=1d0/4d0/s12
     & *((im*h1tZ+h3tZ)*2d0*za(p3,p1)*zb(p4,p5)*zb(p2,p5)
     &  +(im*h2tZ+h4tZ)*za(p3,p4)*za(p1,p5)*zb(p5,p2)*zb(p4,p5)**2)
C (3<-->4)
      anomZgam(1,2,2)=1d0/4d0/s12
     & *((im*h1tZ+h3tZ)*2d0*za(p4,p1)*zb(p3,p5)*zb(p2,p5)
     &  +(im*h2tZ+h4tZ)*za(p4,p3)*za(p1,p5)*zb(p5,p2)*zb(p3,p5)**2)
C Flip_2: (za<-->zb),(1<-->2),(3<-->4) and h1t -> -h1t, h2t -> -h2t
      anomZgam(1,1,1)=1d0/4d0/s12
     & *((-im*h1tZ+h3tZ)*2d0*zb(p4,p2)*za(p3,p5)*za(p1,p5)
     &  +(-im*h2tZ+h4tZ)*zb(p4,p3)*zb(p2,p5)*za(p5,p1)*za(p3,p5)**2)
C (za<-->zb),(1<-->2)
      anomZgam(1,2,1)=1d0/4d0/s12
     & *((-im*h1tZ+h3tZ)*2d0*zb(p3,p2)*za(p4,p5)*za(p1,p5)
...
C     This is complex conjugation
      ...
      anomZZ(2,2,1)=Dconjg(anomZZ(1,1,2))
      anomZZ(2,1,1)=Dconjg(anomZZ(1,2,2))
      anomZZ(2,1,2)=Dconjg(anomZZ(1,2,1))
      anomZZ(2,2,2)=Dconjg(anomZZ(1,1,1))

      anomgamZ(2,2,1)=Dconjg(anomgamZ(1,1,2))
      anomgamZ(2,1,1)=Dconjg(anomgamZ(1,2,2))
      anomgamZ(2,1,2)=Dconjg(anomgamZ(1,2,1))
      anomgamZ(2,2,2)=Dconjg(anomgamZ(1,1,1))

      anomZgam(2,2,1)=Dconjg(anomZgam(1,1,2))
      anomZgam(2,1,1)=Dconjg(anomZgam(1,2,2))
      anomZgam(2,1,2)=Dconjg(anomZgam(1,2,1))
      anomZgam(2,2,2)=Dconjg(anomZgam(1,1,1))
...
c--   correction factor due to final-state Z not being exactly on-shell
      cprop=(s12-s34)/Dcmplx((s12-zmass**2),zmass*zwidth)
