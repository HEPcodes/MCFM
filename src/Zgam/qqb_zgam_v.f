...
c     q(-p1)+qbar(-p2)-->e^-(p3)+e-(p4)+gamma(p5)
     ...
     .              qbqnlo(2,2,2,2),qqbnlo(2,2,2,2)
      ...
      fac=cf*ason2pi*aveqq*8d0*esq**3*xn
      do j=1,2
      qbq(j)=0d0
      qqb(j)=0d0
      do h12=1,2
      ...
      do h5= 1,2
      qbq(j)=qbq(j)
     . +2d0*Dble(Dconjg(qbqlo(j,h12,h34,h5))*qbqnlo(j,h12,h34,h5))
      qqb(j)=qqb(j)
     . +2d0*Dble(Dconjg(qqblo(j,h12,h34,h5))*qqbnlo(j,h12,h34,h5))
      enddo
      enddo
      enddo
      qbq(j)=fac*qbq(j)
...
C  hep-ph/9803250 Eqns 4.6 and 4.7
      ...
      double complex qbqlo(2,2,2,2),qbqnlo(2,2,2,2),fagamma,fbgamma,
     . prp34,vpole,vpl
