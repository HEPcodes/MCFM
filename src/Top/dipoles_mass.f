...
c--  +4*[ln(1-x)/(1-xp)]+2*L/[1-xp]
      ...
      if (vorz .eq. 1) then
        ii_mqq=epinv*(epinv2-L)+0.5d0*L**2+1.5d0*epinv-pisqo6
     .   -epinv*1.5d0
...
c--  +(4*[ln(1-x)/(1-xp)]+2*L/[1-xp])
     ...
     .       -(11d0-two*nf/xn)/6d0*epinv
      ...
      if (vorz .eq. 2) then
        ...
        alfax=alfa/omx
        if (alfax .lt. 1d0) ii_mgg=ii_mgg+Pggreg*dlog(alfax)
        return
      endif
...
c     .   -two/omx*(dlog((2d0-x+mbarsq)/(1d0+mbarsq))+lx)
     ...
     .   +omx-2d0/omx*(lx+dlog((1d0+x*mbarsq+omx)/(1d0+mbarsq)))
         if (alfa .lt. zp)
     .   if_mqq=if_mqq-(two/omx*(dlog(zp*(omx+alfa)/(alfa*(omx+zp))))
     .   +Pqqreg*dlog(zp/alfa))
         return
...
c  -2/omx*(log(x)+log(1+x*mbarsq+omx,1+mbarsq))
         if (alfa .lt. zp)
     .   if_mgg=if_mgg-(two/omx*(dlog(zp*(omx+alfa)/(alfa*(omx+zp))))
     .   +Pggreg*dlog(zp/alfa))
         return
      elseif (vorz .eq. 3) then
C plus contribution
...
CDST(5.60)
c      JaNS=0.5d0*dlog(1d0+mbarsq)-2d0*dlog(mbarsq)*dlog(1d0+mbarsq)
...
c      fi_mqq=-(two*dlog(omx)+1.5d0)/omx
        fi_mqq=2d0/omx*(dlog((1d0+mbarsq)/(omx+x*mbarsq))-1d0)
      endif
