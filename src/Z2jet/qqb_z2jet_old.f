      subroutine qqb_z2jet_old(p,msq)
      implicit none
...
C---exclude the photon pole, 4*mbsq choosen as a scale approx above upsilon 
      ...
      call z2jetsq_old(1,2,3,4,5,6,za,zb,qqbgg)
      call z2jetsq_old(1,5,3,4,2,6,za,zb,qgqg)
      call z2jetsq_old(2,5,3,4,1,6,za,zb,gqqg)
      call z2jetsq_old(5,6,3,4,1,2,za,zb,ggqbq)
...
c      call z2jetsq(5,2,3,4,1,6,za,zb,gqbqbg)
      ...
      do pq=1,2
      do pl=1,2
      qqbgg(pq,pl) =half*aveqq*fac*qqbgg(pq,pl)
...
c---Statistical factor
     ...
     .             +(Q(-k)*q1+R(-k)*l1*prop)**2*gqbqbg(2,1)
      ...
      write(6,*) 'j,k,msq(j,k)',j,k,msq(j,k)
   19 continue
      enddo
      enddo
