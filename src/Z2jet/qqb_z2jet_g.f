...
c                           
     ...
     .                 msqi_gqb(2),msqi_gq(2),ggtemp
      double precision msqn_qq(2,2),msqn_qbqb(2,2),
...
c-qqb
     ...
     .      +dfloat(1+jj(j))*msqn_qqb(jj(j),1)
     .      +dfloat(3-jj(j))*msqn_qqb(jj(j),2))
           else
...
c-qbq
     ...
     .      +dfloat(1+kk(k))*msqn_qbq(kk(k),1)
     .      +dfloat(3-kk(k))*msqn_qbq(kk(k),2))
          else
...
c-qg
     ...
     .      +dfloat(1+jj(j))*msqn_qg(jj(j),1)
     .      +dfloat(3-jj(j))*msqn_qg(jj(j),2)
     . )
c-qbg
      elseif ((j .lt. 0) .and. (k .eq. 0)) then
          msq(j,k)=msq(j,k)
     .     +fac*aveqg*(half*msqi_qbg(-jj(j))
     .      +dfloat(1-jj(j))*msqn_qbg(-jj(j),1)
     .      +dfloat(3+jj(j))*msqn_qbg(-jj(j),2)
     . )
      elseif ((j .eq. 0) .and. (k .gt. 0)) then
c-gq
          msq(j,k)=msq(j,k)
     .      +fac*aveqg*(half*msqi_gq(kk(k))
     .      +dfloat(1+kk(k))*msqn_gq(kk(k),1)
     .      +dfloat(3-kk(k))*msqn_gq(kk(k),2)
     . )
      elseif ((j .eq. 0) .and. (k .lt. 0)) then
c-gqb
          msq(j,k)=msq(j,k)
     .      +fac*aveqg*(half*msqi_gqb(-kk(k))
     .      +dfloat(1-kk(k))*msqn_gqb(-kk(k),1)
     .      +dfloat(3+kk(k))*msqn_gqb(-kk(k),2)
     . )
