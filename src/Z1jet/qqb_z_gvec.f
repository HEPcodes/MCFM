...
C--in is the label of the parton dotted with n
      ...
      double precision z1jetn,prop,fac,p1p2(-1:1,-1:1),n(4)
     ...
     .              +(Q(j)*q1+R(j)*r1*prop)**2)*p1p2(1,-1)
     .             +((Q(j)*q1+L(j)*r1*prop)**2
     .              +(Q(j)*q1+R(j)*l1*prop)**2)*p1p2(-1,1)
      elseif ((j .lt. 0) .and. (k .gt. 0)) then
          msq(j,k)=+((Q(k)*q1+L(k)*l1*prop)**2
     .              +(Q(k)*q1+R(k)*r1*prop)**2)*p1p2(-1,1)
     .             +((Q(k)*q1+L(k)*r1*prop)**2
     .              +(Q(k)*q1+R(k)*l1*prop)**2)*p1p2(1,-1)
      elseif ((j .gt. 0) .and. (k .eq. 0)) then
          msq(j,k)=+((Q(j)*q1+L(j)*l1*prop)**2
     .              +(Q(j)*q1+R(j)*r1*prop)**2)*p1p2(+1,0)
     .             +((Q(j)*q1+L(j)*r1*prop)**2
     .              +(Q(j)*q1+R(j)*l1*prop)**2)*p1p2(-1,0)
     ...
     .              +(Q(-j)*q1+R(-j)*l1*prop)**2)*p1p2(+1,0)
      elseif ((j .eq. 0) .and. (k .gt. 0)) then
          msq(j,k)=+((Q(k)*q1+L(k)*l1*prop)**2
     .              +(Q(k)*q1+R(k)*r1*prop)**2)*p1p2(0,+1)
     .             +((Q(k)*q1+L(k)*r1*prop)**2
     .              +(Q(k)*q1+R(k)*l1*prop)**2)*p1p2(0,-1)
