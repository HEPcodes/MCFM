      subroutine KMgg2c3(mt,zp1,zp2,zp3,zp4,e1,e2,Ubm,Vm,
     & KMgg2c3ba,KMgg2c3dd)
      implicit none
      include 'constants.f'
      include 'eplog.f'
...
c--- multiply by (1+ep^2*zeta2)
     ...
     &  +6d0*log(-t1/mtsq)*mtsq/tt-2d0*fll)/2d0
      if (scheme .eq. 'dred') then
        KMgg2c3ba=KMgg2c3ba+Nc/2d0
      endif      
      KMgg2c3ba=KMgg2c3ba*Bt(zp1,zp2,zp3,zp4,e1,e2,Ubm,Vm)
      
      KMgg2c3ba=KMgg2c3ba+3d0*im*(
     ...
     & +mt*cdot(e1,zp3)*string2(Ubm,zp1,e2,Vm)*(
