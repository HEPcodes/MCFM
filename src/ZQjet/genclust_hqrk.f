...
c--- WQQj
************************************************************************
c--- particle assignments are: 5(bq) 6(ba) 7(pp)
      if ((nproc .eq. 24) .or. (nproc .eq. 29)) then
c--- first ensure that all partons are well-separated
        if ( (Ry(q,5,6) .lt. Rbbmin)
     .  .or. (Ry(q,5,7) .lt. Rmin)
     .  .or. (Ry(q,6,7) .lt. Rmin) ) then
          jets=-1
          return
        endif
        do nu=1,4
        ...
        jetlabel(3)='pp'
        jets=3        
c--- check that all jets pass pt and rapidity cuts
        if ( (pt(5,qfinal) .lt. ptbjetmin)
     .  .or. (ayrap(5,qfinal) .gt. etabjetmax)) then
          jets=-1
          return
        endif
        if ( (pt(6,qfinal) .lt. ptbjetmin)
     .  .or. (ayrap(6,qfinal) .gt. etabjetmax)) then
          jets=-1
          return
        endif
        if ( (pt(7,qfinal) .lt. ptjetmin)
     .  .or. (ayrap(7,qfinal) .gt. etajetmax)) then
          jets=-1
          return
        endif
      endif
...
************************************************************************
c--- WQj - basic LO process
************************************************************************
c--- particle assignments are: 5(bq) 6(pp)
      if ((nproc .eq. 311) .or. (nproc .eq. 316)) then
c--- first ensure that ba and jet are well-separated
        if (Ry(q,5,6) .lt. Rmin) then
          jets=-1
          return         
        endif
        do nu=1,4
        ...
        jetlabel(2)='pp'
        jets=2        
c--- check that both jets pass pt and rapidity cuts
        if ( (pt(5,qfinal) .lt. ptbjetmin)
     .  .or. (ayrap(5,qfinal) .gt. etabjetmax)) then
          jets=-1
          return
        endif
        if ( (pt(6,qfinal) .lt. ptjetmin)
     .  .or. (ayrap(6,qfinal) .gt. etajetmax)) then
          jets=-1
          return
        endif
      endif
...
c--- particle assignments are: 5(bq) 6(ba) 7(pp)
      if ((nproc .eq. 313) .or. (nproc .eq. 318)) then
c--- first ensure that bq and ba should be merged together
        if (Ry(q,5,6) .gt. Rbbmin) then
          jets=-1
          return
        endif
c--- merge b's and shuffle down
        do nu=1,4
          qfinal(5,nu)=q(5,nu)+q(6,nu)
          qfinal(6,nu)=q(7,nu)
        enddo        
        jetlabel(1)='bq'
        jetlabel(2)='pp'
        jets=2        
c--- check to see if bq and jet are too close
        if (Ry(qfinal,5,6) .lt. Rmin) then
          jets=-1
          return
        endif
c--- check that both jets pass pt and rapidity cuts
...
c--- WQjj
************************************************************************
c--- particle assignments are: 5(bq) 6(pp) 7(pp)
      if ((nproc .eq. 314) .or. (nproc .eq. 319)) then
c--- first ensure that all partons are well-separated
        if ( (Ry(q,5,6) .lt. Rmin)
     .  .or. (Ry(q,5,7) .lt. Rmin)
     .  .or. (Ry(q,6,7) .lt. Rmin) ) then
