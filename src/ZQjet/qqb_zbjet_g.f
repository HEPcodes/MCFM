...
c--- calcule g-H and g-q amplitudes       
      if (isub .eq. 1) call msq_ZqqQQg_noid(5,2,7,6,1,4,3,msq_gH)
      if (isub .eq. 2) call msq_ZqqQQg_noid(5,6,7,2,1,4,3,msq_gq)
c--- calcule H-g and q-g amplitudes       
      if (isub .eq. 1) call msq_ZqqQQg_noid(5,1,7,6,2,4,3,msq_Hg)
      if (isub .eq. 2) call msq_ZqqQQg_noid(5,6,7,1,2,4,3,msq_qg)
c--- calcule g-Hb and g-qb amplitudes       
      if (isub .eq. 1) call msq_ZqqQQg_noid(2,5,7,6,1,4,3,msq_gHb)
      if (isub .eq. 2) call msq_ZqqQQg_noid(5,6,2,7,1,4,3,msq_gqb)
c--- calcule Hb-g and qb-g amplitudes       
      if (isub .eq. 1) call msq_ZqqQQg_noid(1,5,7,6,2,4,3,msq_Hbg)
      if (isub .eq. 2) call msq_ZqqQQg_noid(5,6,1,7,2,4,3,msq_qbg)
