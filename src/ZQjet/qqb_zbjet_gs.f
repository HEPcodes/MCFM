...
c      c15=c15+dfloat(j*10+k+99)*(msq15_2(j,k)-
c     . (+msq1b_2(3,0,j,k)+msq1b_2(3,1,j,k)+msq1b_2(3,2,j,k)))
c      c15v=c15v+dfloat(j*10+k+99)*(msq15_2v(j,k)-
c     . (+msq1b_2v(3,0,j,k)+msq1b_2v(3,1,j,k)+msq1b_2v(3,2,j,k)))
c      c25=c25+dfloat(j*10+k+99)*(msq25_1(j,k)-
c     . (+msq2b_1(3,0,j,k)+msq2b_1(3,1,j,k)+msq2b_1(3,2,j,k)))
c      c25v=c25v+dfloat(j*10+k+99)*(msq25_1v(j,k)-
c     . (+msq2b_1v(3,0,j,k)+msq2b_1v(3,1,j,k)+msq2b_1v(3,2,j,k)))
c      c16=c16+dfloat(j*10+k+99)*(msq16_2(j,k)-
c     . (+msq1b_2(1,0,j,k)+msq1b_2(1,1,j,k)+msq1b_2(1,2,j,k)))
c      c16v=c16v+dfloat(j*10+k+99)*(msq16_2v(j,k)-
c     . (+msq1b_2v(1,0,j,k)+msq1b_2v(1,1,j,k)+msq1b_2v(1,2,j,k)))
c      c26=c26+dfloat(j*10+k+99)*(msq26_1(j,k)-
c     . (+msq2b_1(1,0,j,k)+msq2b_1(1,1,j,k)+msq2b_1(1,2,j,k)))
c      c26v=c26v+dfloat(j*10+k+99)*(msq26_1v(j,k)-
c     . (+msq2b_1v(1,0,j,k)+msq2b_1v(1,1,j,k)+msq2b_1v(1,2,j,k)))
c      c571v=c571v+dfloat(j*10+k+99)*(msq57_1v(j,k)-
c     . (+msqba_1v(3,0,j,k)+msqba_1v(3,1,j,k)+msqba_1v(3,2,j,k)))
c      c671v=c671v+dfloat(j*10+k+99)*(msq67_1v(j,k)-
c     . (+msqba_1v(6,0,j,k)+msqba_1v(6,1,j,k)+msqba_1v(6,2,j,k)))
c      c572v=c572v+dfloat(j*10+k+99)*(msq57_2v(j,k)-
c     . (+msqbc_2v(5,0,j,k)+msqbc_2v(5,1,j,k)+msqbc_2v(5,2,j,k)))
c      c672v=c672v+dfloat(j*10+k+99)*(msq67_2v(j,k)-
c     . (+msqbc_2v(1,0,j,k)+msqbc_2v(1,1,j,k)+msqbc_2v(1,2,j,k)))
c
c      do i=0,2
c      c57=c57+dfloat(j*10+k+99)*(m57_6(i,j,k)-msqab_c(2,i,j,k))
c      c57v=c57v+dfloat(j*10+k+99)*(m57_6v(i,j,k)-msqab_cv(2,i,j,k))
c      c67=c67+dfloat(j*10+k+99)*(m67_5(i,j,k)-msqab_c(4,i,j,k))
c      c67v=c67v+dfloat(j*10+k+99)*(m67_5v(i,j,k)-msqab_cv(4,i,j,k))
c      c175=c175+dfloat(j*10+k+99)*(m17_5(i,j,k)-msq1a_b(3,i,j,k))
c      c175v=c175v+dfloat(j*10+k+99)*(m17_5v(i,j,k)-msq1a_bv(3,i,j,k))
c      c176=c176+dfloat(j*10+k+99)*(m17_6(i,j,k)-msq1a_b(6,i,j,k))
c      c176v=c176v+dfloat(j*10+k+99)*(m17_6v(i,j,k)-msq1a_bv(6,i,j,k))
c      c275=c275+dfloat(j*10+k+99)*(m27_5(i,j,k)-msq2c_b(5,i,j,k))
c      c275v=c275v+dfloat(j*10+k+99)*(m27_5v(i,j,k)-msq2c_bv(5,i,j,k))
c      c276=c276+dfloat(j*10+k+99)*(m27_6(i,j,k)-msq2c_b(1,i,j,k))
c      c276v=c276v+dfloat(j*10+k+99)*(m27_6v(i,j,k)-msq2c_bv(1,i,j,k))
c      enddo
c      enddo
c      enddo
c
...
c---Q-Q contribution (isub=1 only)
      if     ((isub .eq. 1) .and. (j .gt. 0) .and. (k.gt.0)) then
        if     (j .eq. +flav) then
...
c--- Q-g contribution (isub=1 only)
        if     ((isub .eq. 1) .and. (j .eq. +flav)) then
          msq(22,j,k)=msq(22,j,k)+(
     .     +sub26_1(qg)*msq26_1(j,1)*2d0
     .     +sub26_1(qg)*msq26_1(j,2)*dfloat(flav-3))
          msq(23,j,k)=msq(23,j,k)+(
     .     +sub27_1(qg)*msq27_1(j,-1)*2d0
     .     +sub27_1(qg)*msq27_1(j,-2)*dfloat(flav-3))
          msq(12,j,k)=msq(12,j,k)+dfloat(flav-1)*(
     .     +sub67_1(gq)*msq17_6(j,k)-sub67_1v*msq67_1v(j,k))
c--- q-g contribution (isub=2 only)
        elseif ((isub .eq. 2) .and. (j .ne. +flav)) then
          msq(22,j,k)=msq(22,j,k)+(
...
c--- g-Q contribution (isub=1 only)
     ...
     .     +sub16_2(qg)*msq16_2(1,k)*2d0
     .     +sub16_2(qg)*msq16_2(2,k)*dfloat(flav-3))
          msq(20,j,k)=msq(20,j,k)+(
     .     +sub17_2(qg)*msq17_2(-1,k)*2d0
     .     +sub17_2(qg)*msq17_2(-2,k)*dfloat(flav-3))
          msq(12,j,k)=msq(12,j,k)+dfloat(flav-1)*(
     .     +sub67_1(gq)*msq17_6(j,k)-sub67_1v*msq67_1v(j,k))
c--- g-q contribution (isub=2 only)
        elseif ((isub .eq. 2) .and. (k .ne. +flav)) then
          msq(19,j,k)=msq(19,j,k)+(
