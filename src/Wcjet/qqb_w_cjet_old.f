...
C for nwz=+1
c     u(-p1)+dbar(-p2)--> W^+(n(p3)+e^+(p4))   + cbar(p5)
C For nwz=-1
c     d(-p1)+ubar(-p2)--> W^-(e^-(p3)+nbar(p4))+ c(p5) 
c---
      include 'constants.f'
      include 'ckm.f'
      include 'ewcouple.f'
...
c---calculate the propagator
      ...
      gqbWqb=-aveqg*fac*w1cjet(5,2,3,4,1)
      qgWq=  -aveqg*fac*w1cjet(1,5,3,4,2)
      qbgWqb=-aveqg*fac*w1cjet(5,1,3,4,2)
...
C     Matrix element squared for s(1) cbar(2) -> e-(3) nu(4) g(5)
      ...
      w1cjet=-2d0*s(j4,j1)*mc**2*(s(j3,j2)+s(j3,j5))/s(j2,j5)/s(j2,j5)
     . -s(j3,j2)/s(j2,j5)
