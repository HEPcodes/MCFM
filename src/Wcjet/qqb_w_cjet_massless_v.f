      subroutine qqb_w_cjet_massless_v(p,msq)
      implicit none
...
c      data iqqbg/1,2,3,4,5/
      data iqgq/1,5,3,4,2/
      data igqq/2,5,3,4,1/

c      data iqbqg/2,1,3,4,5/
...
c--- calculate lowest order
      call qqb_w_cjet_massless(p,msq0)
...
c--- calculate propagator
      ...
      if ((j .gt. 0) .and. (k .eq. 0)) then
          msq(j,k)=Vsq(j,-4)*qgWq-subuv*msq0(j,k)
      elseif ((j .lt. 0) .and. (k .eq. 0)) then
          msq(j,k)=Vsq(j,+4)*qbgWqb-subuv*msq0(j,k)
      elseif ((j .eq. 0) .and. (k .gt. 0)) then
          msq(j,k)=Vsq(-4,k)*gqWq-subuv*msq0(j,k)
      elseif ((j .eq. 0) .and. (k .lt. 0)) then
          msq(j,k)=Vsq(+4,k)*gqbWqb-subuv*msq0(j,k)
      endif
